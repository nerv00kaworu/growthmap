...
type HomeCopy={
  ...
  demoLabel:string;demoNote:string;play:string;pause:string;
  ...
  closing:string;closingLead:string;explore:string;tree:{label:string;type:string}[];
};

const zhTW:HomeCopy={
  eyebrow:'人類與 Agent 共用的專案地圖',
  title:'不管從畫面操作，還是從對話開始，專案都長在同一張地圖。',
  ...
  demoLabel:'概念示範 · 非實機畫面',demoNote:'兩支動畫使用同一個「個人記帳 App」，只改變專案進入 GrowthMap 的方式。',play:'播放概念動畫',pause:'暫停概念動畫',

...
test('buy remains offline and disabled with no real order path',()=>{const page=read('app/(localized)/[locale]/buy/page.tsx'),core=read('content/i18n.ts');assert.ok(page.includes('disabled aria-disabled="true"'));for(const forbidden of ['fetch(','axios','wallet','credentials','createOrder','onClick='])assert.ok(!page.includes(forbidden),forbidden);for(const truth of ['no wallet connection','creates no real order'])assert.ok(core.includes(truth),truth)});

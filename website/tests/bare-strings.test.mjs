...
const files=[
  'app/(localized)/[locale]/layout.tsx',...fs.readdirSync(path.join(root,'app/(localized)/[locale]'),{withFileTypes:true}).filter(x=>x.isDirectory()&&fs.existsSync(path.join(root,`app/(localized)/[locale]/${x.name}/page.tsx`))).map(x=>`app/(localized)/[locale]/${x.name}/page.tsx`),

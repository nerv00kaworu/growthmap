...
const media=[
  {video:'/media/home/gui-growth.webm',poster:'/media/home/gui-growth-poster.jpg'},
  {video:'/media/home/agent-conversation.webm',poster:'/media/home/agent-conversation-poster.jpg'}
] as const;

function DemoVideo({index,label,play,pause}:{index:number;label:string;play:string;pause:string}){
  const ref=useRef<HTMLVideoElement>(null);const [playing,setPlaying]=useState(false);
  useEffect(()=>{const video=ref.current;if(!video)return;const observer=new IntersectionObserver(entries=>{if(entries[0]?.isIntersecting){void video.play().catch(()=>{});}else video.pause();},{threshold:.35});observer.observe(video);return()=>observer.disconnect()},[]);
  return <figure className="home-video-card"><video ref={ref} muted loop playsInline preload="metadata" poster={media[index].poster} aria-label={label} onPlay={()=>setPlaying(true)} onPause={()=>setPlaying(false)}><source src={media[index].video} type="video/webm"/></video><button type="button" className="home-video-toggle" aria-label={playing?pause:play} onClick={()=>{const video=ref.current;if(!video)return;if(video.paused)void video.play();else video.pause()}}>{playing?'Ⅱ':'▶'}</button><figcaption>{label}</figcaption></figure>
}

export function HomeLanding({locale}:{locale:Locale}){const copy=homeContent[locale];return <>
  <section className="home-hero"><div><p className="eyebrow">{copy.eyebrow}</p><h1>{copy.title}</h1><p className="lead">{copy.lead}</p><div className="actions"><a className="button" href="#two-workflows">{copy.primary}</a><a className="textlink" href="#project-example">{copy.secondary} →</a></div><p className="home-micro">{copy.micro}</p></div><DemoVideo index={0} label={`${copy.paths[0].label} · ${copy.demoLabel}`} play={copy.play} pause={copy.pause}/></section>
  <section className="home-section" id="two-workflows"><header className="home-section-head"><p className="eyebrow">{copy.pathsEyebrow}</p><h2>{copy.pathsTitle}</h2><p>{copy.pathsLead}</p></header><div className="home-paths">{copy.paths.map((path,index)=><article className="home-path" key={path.label}><span>{path.label}</span><h3>{path.title}</h3><p>{path.body}</p><div className="home-path-flow">{path.flow.map((item,i)=><span key={item}>{i>0&&<i>→</i>}<b>{item}</b></span>)}</div><DemoVideo index={index} label={`${path.label} · ${copy.demoLabel}`} play={copy.play} pause={copy.pause}/></article>)}</div><p className="home-converge">{copy.converge}</p><p className="home-demo-note">{copy.demoNote}</p></section>

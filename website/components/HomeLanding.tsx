...
function DemoVideo({kind,label,play,pause}:{kind:keyof typeof media;label:string;play:string;pause:string}){
  ...
  return <figure className="home-video-card"><video ref={ref} muted loop playsInline preload="metadata" poster={media[kind].poster} aria-label={label} onPlay={()=>setPlaying(true)} onPause={()=>setPlaying(false)}><source src={media[kind].video} type="video/webm"/></video><button type="button" className="home-video-toggle" aria-label={playing?pause:play} onClick={()=>{const video=ref.current;if(!video)return;if(video.paused)void video.play();else video.pause()}}>{playing?'Ⅱ':'▶'}</button><figcaption>{label}</figcaption></figure>
}

export function HomeLanding({locale}:{locale:Locale}){const copy=homeContent[locale];return <>
  <section className="home-hero"><div><p className="eyebrow">{copy.eyebrow}</p><h1>{copy.title}</h1><p className="lead">{copy.lead}</p><div className="actions"><a className="button" href="#two-workflows">{copy.primary}</a><a className="textlink" href="#project-example">{copy.secondary} →</a></div><p className="home-micro">{copy.micro}</p></div><DemoVideo kind="overview" label={copy.overviewLabel} play={copy.play} pause={copy.pause}/></section>
  <section className="home-section" id="two-workflows"><header className="home-section-head"><p className="eyebrow">{copy.pathsEyebrow}</p><h2>{copy.pathsTitle}</h2><p>{copy.pathsLead}</p></header><div className="home-paths">{copy.paths.map((path,index)=><article className="home-path" key={path.label}><span>{path.label}</span><h3>{path.title}</h3><p>{path.body}</p><div className="home-path-flow">{path.flow.map((item,i)=><span key={item}>{i>0&&<i>→</i>}<b>{item}</b></span>)}</div><DemoVideo kind={index===0?'gui':'conversation'} label={`${path.label} · ${copy.demoLabel}`} play={copy.play} pause={copy.pause}/></article>)}</div><p className="home-converge">{copy.converge}</p><p className="home-demo-note">{copy.demoNote}</p></section>

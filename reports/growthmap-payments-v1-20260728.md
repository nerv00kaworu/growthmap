...
## R11 signed revocation/check-in candidate closure (current)

The local candidate now closes the prior gap where an admin `revoke` changed only the order row while an already imported perpetual license remained independently valid. Schema **v7** adds immutable, checkpoint-authenticated signed revocation assertions. Assertions use a domain-separated canonical Ed25519 payload bound to exact schema/type/product, major version, license ID, canonical UTC revocation time, a positive monotonically increasing sequence, and a strict reason enum. Revoke creation is transactional and idempotent; revoked recovery never returns an active license. New licenses carry a bounded 30-day signed check-in deadline, while legacy signed licenses bootstrap at most 30 days from a protected first observation rather than from attacker-controlled license timestamps.

Desktop candidate controls verify signatures/bindings and persist paid freshness, revocation, commercial-history/presence witnesses, and installation identity through OS-protected receipts plus atomic local files. Clock high-water is checkpointed for paid/check-in-required/expired observations; signed revocation is applied before legacy bootstrap promotion. All raw trust JSON boundaries reject duplicate and casefold-colliding keys before ordinary parsing. Missing, corrupt, deleted, rolled-back, or incoherent evidence fails closed while a genuinely never-commercial/no-license/trial profile remains usable. The software-only boundary is explicit: coordinated restoration of a mutually consistent earlier snapshot of every matching application artifact and OS credential-store record still requires an external monotonic/HSM or authenticated online witness to prevent.

Independent review proceeded through four remediation rounds. It reproduced and closed: deletion of both primary receipts; legacy bootstrap reset; future-clock rollback; bool/timestamp/order ambiguities; duplicate-key collapse; missing package entries; commercial history/witness inconsistency; expired-paid identity loss; legacy revocation bypass; strict-parser gaps in protected receipts; and malformed installation identity being misclassified as a fresh install. Fresh final review returned **PASS** for exact candidate `9e675bcb21e273566bfe7c0ec4d6a520323ded22`. Local evidence included payments+backend **87 passed**, desktop **96 passed**, frontend lint/typecheck/build PASS, and focused independent cryptographic/startup/reproducer suites.

Exact GitHub Actions run [`30425575435`](https://github.com/nerv00kaworu/growthmap/actions/runs/30425575435) completed **PASS** at merged SHA `9e675bcb21e273566bfe7c0ec4d6a520323ded22`: Windows backend/frontend/desktop tests, sidecar build, frozen provenance, isolated packaged E2E, unsigned NSIS, adversarial DB/crash recovery, required revocation/history modules, final production ASAR/resources/E2E exclusion, installer SHA-256 and artifact upload all passed. Authenticode/updater publication verification remained intentionally skipped without production signing inputs.

The **local/offline signed revocation/check-in candidate gate is PASS**. Production/public release remains **NO-GO** for official provider/reconciler integration, production Base recipient and signing-key provider, real HTTPS payment/wallet surfaces, production admin auth/session/recovery operations, reviewed PayPal merchant operations, an external monotonic/HSM or authenticated online rollback witness, and legal/config/Authenticode/updater publication inputs.

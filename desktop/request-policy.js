...
function createRequestPolicy({getBaseUrl,getToken,token,csp}){
 function before(details){if(origin(details.url)!==getBaseUrl())return {cancel:true};return {requestHeaders:{...(details.requestHeaders||{}),Authorization:`Bearer ${getToken?getToken():token}`}};}

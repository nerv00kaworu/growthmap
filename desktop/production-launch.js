...
async function completeProductionLaunch({prepare,hasStore,bind,makeStore,hydrateAndSeal,enable}){
 ...
 try{
  ...
  const failure=new Error('Production launch barrier failed');Object.defineProperties(failure,{launchBarrierStage:{value:stage},nativeOperation:{value:diagnosticValue(error,'nativeOperation')},stage:{value:diagnosticValue(error,'stage')},credentialStage:{value:diagnosticValue(error,'credentialStage')}});
  if(rollbackError){const rollbackOperation=diagnosticValue(rollbackError,'nativeOperation'),rollbackStage=diagnosticValue(rollbackError,'stage'),rollbackFailure=new Error('Authority rollback failed');Object.defineProperties(rollbackFailure,{nativeOperation:{value:rollbackOperation},stage:{value:rollbackStage}});const aggregate=new AggregateError([failure,rollbackFailure],'Launch and authority rollback both failed');Object.defineProperties(aggregate,{launchBarrierStage:{value:stage},credentialStage:{value:failure.credentialStage},rollbackNativeOperation:{value:rollbackOperation},rollbackNativeStage:{value:rollbackStage}});throw aggregate}
  throw failure;
 }
}

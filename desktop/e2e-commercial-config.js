...
const EXACT=['schemaVersion', 'productMajor', 'licensePublicKeyResource', 'licensePublicKeySha256', 'paymentApiOrigin', 'purchasePortalOrigin', 'purchasePortalUrl', 'baseNetwork', 'baseUsdc', 'basePayee', 'earlyLimit', 'earlyPriceMicros', 'regularPriceMicros', 'paypalUrl', 'updateUrl', 'updateOrigin', 'publisher', 'publisherStatus'];
function loadE2ECommercialConfig({isPackaged,resourcesPath}){
 ...
 if(Object.keys(doc).sort().join('\0')!==EXACT.slice().sort().join('\0')||doc.schemaVersion!==2||doc.productMajor!==1||doc.publisherStatus!=='E2E_ONLY'||doc.paymentApiOrigin||doc.purchasePortalOrigin||doc.purchasePortalUrl||doc.paypalUrl||doc.updateUrl||doc.updateOrigin)throw Error('Isolated E2E commercial configuration is invalid');

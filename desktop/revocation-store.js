...
function createRevocationStore({userData,encrypt,decrypt}){
 ...
 function inspect(){
  const a=fs.existsSync(assertion),r=fs.existsSync(receipt),h=history.read();if(h===null)return {state:'invalid',reason:'revocation_history_invalid'};if(!a&&!r)return h.has('revocation')?{state:'invalid',reason:'revocation_evidence_deleted'}:{state:'none'};if(!a||!r||!h.has('revocation'))return {state:'invalid',reason:'revocation_evidence_deleted'};
 ...
 return {inspect,commit,paths:{assertion,receipt,history:history.file,historyPresence:history.presence}};
}

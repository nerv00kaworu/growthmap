...
function createSecretStore({trustedRoot,directory,encrypt,decrypt,putRemote,deleteRemote,recoverRemote,recoverStatus,warn=()=>{},files=secureFiles(trustedRoot||directory,directory)}){
 ...
 const readJournal=name=>{const value=decode(name,'journal');if(!value||value.v!==1||!Number.isSafeInteger(value.revision)||value.revision<1||!['set','delete'].includes(value.operation)||typeof value.id!=='string'||journal(value.id)!==name||(value.operation==='set'&&(typeof value.value!=='string'||!value.value)))throw Error('Invalid credential recovery journal');return value};
 ...
 return{
  ...
  async set(id,value){if(typeof value!=='string'||!value||value.includes('\0'))throw Error('API key is required');try{await putRemote(valid(id),value);files.write(bin(id),encrypt(value));return true}catch{throw Error('API key could not be stored securely')}},
  ...
  async recover(id,revision,operation,value){if(!Number.isSafeInteger(revision)||revision<1||!['set','delete'].includes(operation)||operation==='set'&&(typeof value!=='string'||!value||value.includes('\0')))throw Error('Invalid recovery request');const name=journal(id),desired={v:1,id:valid(id),revision,operation,...(operation==='set'?{value}:{})};if(files.exists(name)){const old=readJournal(name);if(JSON.stringify(old)!==JSON.stringify(desired))throw Error('A different credential recovery is pending')}else files.write(name,encrypt(JSON.stringify(desired)));try{await recoverRemote(id,revision,operation,value);promote(desired);return true}catch{throw Error('Credential recovery could not be completed securely')}},
  ...
  async hydrate(){const names=entries();if(names.some(n=>n.endsWith('.recover')))throw Error('Credential recovery must be reconciled before hydration');for(const name of names.filter(n=>n.endsWith('.bin'))){const id=name.slice(0,-4);let value;try{value=decrypt(files.read(name))}catch{quarantine(name,'corrupt');continue}try{await putRemote(id,value)}catch(error){if(error&&error.statusCode===404)quarantine(name,'stale provider');else warn('One stored provider key could not be loaded. Re-enter it in Settings.')}}}
 };
}

...
function readStableRegularFile(file,{maxBytes=65536,fsImpl=fs}={}){
 ...
 if(!hasNoFollow){const before=fsImpl.lstatSync(file);if(before.isSymbolicLink()||!before.isFile()||(!isWindows&&before.nlink!==1))throw Error('unsafe file');const real=fsImpl.realpathSync.native?fsImpl.realpathSync.native(file):fsImpl.realpathSync(file);if(path.resolve(real)!==path.resolve(file))throw Error('unsafe file');}

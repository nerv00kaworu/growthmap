...
async function recoverReplacement({journal,authority,validate,databasePath,fsyncDir,hooks={},nativeOpen=null,platform=process.platform,windowsReplacementOwner=null}){
 ...
 if(platform==='win32'){if(typeof windowsReplacementOwner.prepareRecovery!=='function'){const e=Error('Windows replacement recovery barrier is unavailable');e.code='REPLACEMENT_TRANSACTION_UNAVAILABLE';throw e}await windowsReplacementOwner.prepareRecovery({transitionId:i.transitionId})}
 ...
 if(i.phase==='intent'&&liveOld&&!old&&!failed&&!quarantine){if(fs.existsSync(i.stagingPath))fs.unlinkSync(i.stagingPath);i=await journal.update(i,{phase:'aborted-durable'});await journal.markTerminalVerified(i);await journal.clear(i);return {recovered:true,outcome:'aborted-before-install',completedJournalConsumed:true};}
 ...
 if(!committedPhase){
  ...
  hooks.beforeRollbackMarker?.();await heldOldLive.assertExact(i.oldEvidence);if(heldFailed)await heldFailed.assertExact(i.newEvidence);if(!await authority.matches(i.authorityPre))throw Error('Credential authority rollback verification failed');i=await journal.update(i,{phase:'rollback-durable'});hooks.afterRollbackMarker?.();if(fs.existsSync(i.stagingPath))fs.unlinkSync(i.stagingPath);await journal.markTerminalVerified(i);await journal.clear(i);return {recovered:true,outcome:'rolled-back',completedJournalConsumed:true};
 }

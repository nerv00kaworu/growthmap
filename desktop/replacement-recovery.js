...
async function recoverReplacement({journal,authority,validate,databasePath,fsyncDir,hooks={},nativeOpen=null,platform=process.platform,windowsReplacementOwner=null}){
 let i=journal.read();if(!i)return {recovered:false};if(platform==='win32'&&!windowsReplacementOwner){const e=Error('Windows replacement recovery owner is unavailable');e.code='REPLACEMENT_TRANSACTION_UNAVAILABLE';throw e}const opened=[];const inspect=async file=>{const x=await evidence(file,validate,{nativeOpen});if(x)opened.push(x);return x};const closeAllOpened=async()=>{let closeError=null;for(const item of opened.splice(0)){try{await item.close()}catch(error){closeError??=error}}if(closeError)throw closeError};try{

...
async function recoverReplacement({journal,authority,validate,databasePath,fsyncDir,hooks={},nativeOpen=null,platform=process.platform,windowsReplacementOwner=null}){
 let i=journal.read();if(!i)return {recovered:false};if(platform==='win32'&&!windowsReplacementOwner){const e=Error('Windows replacement recovery owner is unavailable');e.code='REPLACEMENT_TRANSACTION_UNAVAILABLE';throw e}const opened=[];const inspect=async file=>{const x=await evidence(file,validate,{nativeOpen,platform});if(x)opened.push(x);return x};const closeAllOpened=async()=>{let closeError=null;for(const item of opened.splice(0)){try{await item.close()}catch(error){closeError??=error}}if(closeError)throw closeError};try{
 ...
 const oldAuthority=authority.matches(i.authorityPre),newAuthority=i.authorityNew&&authority.matches(i.authorityNew);
 if(!oldAuthority&&!newAuthority)throw Error('Replacement journal and credential authority disagree');
 if(i.phase==='intent'&&liveOld&&!old&&!failed&&!quarantine){if(fs.existsSync(i.stagingPath))fs.unlinkSync(i.stagingPath);i=journal.update(i,{phase:'aborted-durable'});journal.markTerminalVerified(i);return {recovered:true,outcome:'aborted-before-install',completedJournalRetained:true};}
 ...
 if(!committedPhase){
  ...
  if(liveNew&&oldOld){if(failed)throw Error('Replacement failed evidence path already exists');if(platform==='win32')await closeAllOpened();if(platform==='win32')await windowsReplacementOwner.renameExact({action:'rollback-live-to-failed',databasePath,transitionId:i.transitionId,expected:i.newEvidence});else fs.renameSync(databasePath,i.failedPath);hooks.afterLiveToFailed?.();i=journal.update(i,{phase:'rollback-live-moved'});}
  ...
  if(!exact(value(nowLive),i.oldEvidence)){
   ...
   if(platform==='win32')await windowsReplacementOwner.renameExact({action:'rollback-old-to-live',databasePath,transitionId:i.transitionId,expected:i.oldEvidence});else fs.renameSync(i.oldPath,databasePath);hooks.afterOldToLive?.();i=journal.update(i,{phase:'rollback-old-live'});
  }
  ...
  await fsyncDir(path.dirname(databasePath));hooks.afterRollbackFsync?.();await heldOldLive.assertExact(i.oldEvidence);if(heldFailed)await heldFailed.assertExact(i.newEvidence);i=journal.update(i,{phase:'rollback-db-durable'});
  await heldOldLive.assertExact(i.oldEvidence);if(!authority.matches(i.authorityPre))authority.restore(i.authorityPre);hooks.afterAuthorityRestore?.();await heldOldLive.assertExact(i.oldEvidence);
  if(!authority.matches(i.authorityPre))throw Error('Credential authority rollback verification failed');
  hooks.beforeRollbackMarker?.();await heldOldLive.assertExact(i.oldEvidence);if(heldFailed)await heldFailed.assertExact(i.newEvidence);if(!authority.matches(i.authorityPre))throw Error('Credential authority rollback verification failed');i=journal.update(i,{phase:'rollback-durable'});hooks.afterRollbackMarker?.();if(fs.existsSync(i.stagingPath))fs.unlinkSync(i.stagingPath);journal.markTerminalVerified(i);return {recovered:true,outcome:'rolled-back',completedJournalRetained:true};
 }
 ...
 if(initial){if(platform==='win32')await closeAllOpened();if(platform==='win32')await windowsReplacementOwner.renameExact({action:'cleanup-old-to-quarantine',databasePath,transitionId:i.transitionId,expected:i.oldEvidence});else fs.renameSync(i.oldPath,i.quarantinePath);hooks.afterOldQuarantine?.();i=journal.update(i,{phase:'cleanup-old-moved'});}
 if(platform==='win32')await closeAllOpened();else{await old?.close();await quarantine?.close()}let heldLive=await inspect(databasePath),q=await inspect(i.quarantinePath);if(!exact(value(heldLive),i.newEvidence)||!exact(value(q),i.oldEvidence)||!authority.matches(i.authorityNew))throw Error('Committed replacement evidence changed');
 await fsyncDir(path.dirname(databasePath));hooks.afterQuarantineFsync?.();await heldLive.assertExact(i.newEvidence);await q.assertExact(i.oldEvidence);if(!authority.matches(i.authorityNew))throw Error('Committed replacement authority changed');i=journal.update(i,{phase:'cleanup-durable'});hooks.afterCleanupMarker?.();await heldLive.assertExact(i.newEvidence);await q.assertExact(i.oldEvidence);if(!authority.matches(i.authorityNew))throw Error('Committed replacement authority changed');journal.markTerminalVerified(i);return {recovered:true,outcome:'committed',completedJournalRetained:true};

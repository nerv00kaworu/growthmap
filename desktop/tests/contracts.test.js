...
test('R39 release chain binds exact source closure, lazy replacement policy, and real-main gates',()=>{
 ...
 assert.match(workflow,/Windows replacement fail-closed policy acceptance[\s\S]{0,100}timeout-minutes: 5/);assert.match(workflow,/R39 packaged renderer and real-main lifecycle E2E[\s\S]{0,100}timeout-minutes: 9/);assert.doesNotMatch(workflow,/r39-native-\*\.log/);assert.match(workflow,/growthmap-r39-renderer\.png/);

...
test('PayPal purchase IPC uses the pinned validator rather than self-derived origin trust',()=>{assert.match(main,/rail==='paypal'\?validPaypalUrl\(url\)/);assert.doesNotMatch(main,/paypal'.*new URL\(url\)\.origin/);});

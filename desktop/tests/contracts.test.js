...
test('R39 release chain binds exact source closure and mandatory Windows native/real-main gates',()=>{
 ...
 for(const file of ['windows-native-broker.test.js','windows-native-evidence.test.js','windows-native-replacement.test.js','windows-startup-recovery.test.js','windows-r311-broker-gate.js','windows-r312-security-matrix.js','windows-r313-security-matrix.js','windows-r316-startup-acceptance.js','windows-r318-hard-exit-acceptance.js','windows-r39-broker-smoke.js','windows-r39-acceptance.js','windows-r39-manager-acceptance.js']){assert(fs.existsSync(path.join(root,'desktop/tests',file)),file);assert.ok(workflow.includes(file),file);}
 ...
 assert.match(workflow,/timeout-minutes: 20/);assert.match(workflow,/r39-native-\*\.log/);assert.match(workflow,/growthmap-r39-renderer\.png/);

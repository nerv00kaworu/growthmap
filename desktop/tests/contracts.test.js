const test=require('node:test'),assert=require('node:assert/strict'),fs=require('node:fs'),path=require('node:path');const main=fs.readFileSync(require.resolve('../main.js'),'utf8'),preload=fs.readFileSync(require.resolve('../preload.js'),'utf8'),pkg=require('../package.json');test('shell hardening',()=>{for(const x of ['requestSingleInstanceLock','nodeIntegration:false','contextIsolation:true','sandbox:true','setWindowOpenHandler','will-navigate','127.0.0.1'])assert.match(main,new RegExp(x));});test('token header only',()=>{const authority=fs.readFileSync(require.resolve('../session-authority.js'),'utf8');assert.match(authority,/randomBytes\(32\)/);assert.match(main,/sessionAuthority\.begin\(\)/);assert.match(main,/sessionAuthority\.commit\(candidate\)/);assert.match(main,/Authorization/);assert.doesNotMatch(main,/localStorage|searchParams/);assert.doesNotMatch(preload,/session|token|Authorization/i);});test('desktop update and database recovery IPC contracts are exposed',()=>{for(const value of ["'updates:check'","'database:backup'","'database:list-backups'","'database:reveal'","'database:import'","'database:restore'"])assert.match(preload,new RegExp(value));assert.match(main,/updateRecovery/);assert.match(main,/method==='restore'&&!updateRecovery/);const page=fs.readFileSync(require('node:path').resolve(__dirname,'../../src/frontend/src/app/page.tsx'),'utf8');assert.match(page,/data-testid="check-updates-button"/);assert.match(page,/growthmapDesktop\.updates\.check\(\)/);});
test('creator/about and unsigned manual-update identity are trusted-main owned',()=>{const identity=require('../product-identity.json'),page=fs.readFileSync(path.resolve(__dirname,'../../src/frontend/src/app/page.tsx'),'utf8');assert.equal(identity.creatorDisplayName,'月影塵 (nerv00kaworu)');assert.equal(identity.status,'UNSIGNED_BY_OWNER_CHOICE');assert.equal(identity.updateMode,'manual');assert.equal(identity.releasePageUrl,'https://github.com/nerv00kaworu/growthmap/releases');assert.match(main,/'app:info'/);assert.match(main,/'app:open'/);assert.match(preload,/appInfo:Object\.freeze/);assert.match(page,/data-testid="about-growthmap-dialog"/);assert.match(page,/未知發行者/);assert.match(page,/官方 Releases/);assert.doesNotMatch(preload,/creatorDisplayName|releasePageUrl|copyright/);});
...
test('production package verifier uses a CRLF fixture and checks actual ASAR/resource boundaries',()=>{
 ...
 for(const entry of ['updater.js','update-recovery.js','update-policy.js','managed-backup.js','startup-verdict.js','commercial-config.js','product-identity.json','revocation-store.js','license-freshness-store.js','protected-history.js','strict-json.js'])assert.ok(fixture.requiredAsarEntries.includes(entry),`missing ASAR contract ${entry}`);
...
test('pending-update lock precedes recovery, schema, migration backup and writable policy',()=>{const source=fs.readFileSync(require('node:path').join(__dirname,'../main.js'),'utf8'),body=source.slice(source.indexOf('async function prepareAndStart'),source.indexOf('async function launch'));const pending=body.indexOf('if(pending)'),verify=body.indexOf('verifyPending'),recover=body.indexOf('recoverStartup'),schema=body.indexOf('schemaStatus'),backup=body.indexOf('migrationBackup');assert(pending>=0&&verify>pending&&recover>verify&&schema>recover&&backup>schema);assert.match(body,/await stopCandidateAndInvalidate\(/);});

...
test('release workflows pin actions and ASAR verification is repository-local and fail-closed',()=>{
 ...
 for(const file of workflows){const source=fs.readFileSync(path.resolve(__dirname,'../../.github/workflows',file),'utf8');for(const match of source.matchAll(/uses:\s*([^\s#]+)/g)){assert.match(match[1],/@[0-9a-f]{40}$/,`${file} has floating action ${match[1]}`);assert.match(source.slice(match.index,source.indexOf('\n',match.index)),/# v\d+$/,`${file} action pin lacks version comment`);}}
 const resolver=fs.readFileSync(path.resolve(__dirname,'../../.github/workflows/scripts/resolve-local-asar.ps1'),'utf8');assert.match(resolver,/desktop\/package-lock\.json/);assert.match(resolver,/Join-Path \$nodeModulesRoot '@electron\/asar'/);assert.match(resolver,/Join-Path \$expectedPackageRoot 'package\.json'/);assert.match(resolver,/declaredVersion -cne '4\.0\.1'/);assert.match(resolver,/lockedVersion -cne \$declaredVersion/);assert.match(resolver,/installed\.version -cne \$lockedVersion/);assert.match(resolver,/IsPathFullyQualified\(\$rawBin\)/);assert.match(resolver,/rawBin -match/);assert.match(resolver,/without traversal/);assert.match(resolver,/StartsWith\('\.\/'\)/);assert.match(resolver,/normalizedBin -cne 'bin\/asar\.mjs'/);assert.match(resolver,/expectedPackageRoot/);assert.match(resolver,/nodeModulesRoot/);
...
test('R29 candidate metadata is unreleased Windows x64 unsigned-commercial and integrity-verifiable',()=>{
 const manifest=JSON.parse(fs.readFileSync(path.resolve(__dirname,'../../release/windows-r29-candidate.json'),'utf8'));assert.equal(manifest.releaseStatus,'candidate-not-released');assert.equal(manifest.scope.platform,'windows');assert.equal(manifest.scope.arch,'x64');assert.equal(manifest.scope.channel,'unsigned-commercial');assert.equal(manifest.scope.linuxSidecarReleaseGate,false);assert.equal(manifest.source.baseCommit,'e57c3b1a8eed5d79248d8f47d647231ae53e9e4f');assert.equal(manifest.source.tag,null);assert.equal(manifest.source.release,null);assert.equal(Object.keys(manifest.integrity).length,16);const workflow=fs.readFileSync(path.resolve(__dirname,'../../.github/workflows/growthmap-windows-production-personal-v1.yml'),'utf8');assert.match(workflow,/node release\/verify-windows-r29-candidate\.cjs/);assert.equal((workflow.match(/npm audit --audit-level=low/g)||[]).length,2);assert.doesNotMatch(workflow,/npm audit[^\n]*(?:--audit-level=high|--omit)/);const verifyAt=workflow.indexOf('node release/verify-windows-r29-candidate.cjs'),pipAt=workflow.indexOf('python -m pip install'),npmAt=workflow.indexOf('npm ci'),buildAt=workflow.indexOf('Build exact unsigned commercial Windows package');assert.ok(verifyAt>0&&verifyAt<pipAt&&verifyAt<npmAt&&verifyAt<buildAt);const checklist=fs.readFileSync(path.resolve(__dirname,'../../release/WINDOWS-R29-CANDIDATE-CHECKLIST.md'),'utf8');assert.match(checklist,/full dependency graph audit is zero/g);assert.match(checklist,/node release\/verify-windows-r29-candidate\.cjs/);assert.match(fs.readFileSync(path.resolve(__dirname,'../../release/verify-windows-r29-candidate.cjs'),'utf8'),/git',\['show',`HEAD:\$\{relative\}`\]/);assert.match(workflow,/if-no-files-found: ignore/);
});

...
test('R39 release chain binds exact source closure, lazy replacement policy, and real-main gates',()=>{
 ...
 for(const file of ['windows-native-broker.test.js','windows-native-evidence.test.js','windows-native-replacement.test.js','windows-startup-recovery.test.js','database-manager-windows-preflight.test.js']){assert(fs.existsSync(path.join(root,'desktop/tests',file)),file);assert.ok(workflow.includes(file),file);}for(const runtimeGate of ['windows-r311-broker-gate.js','windows-r312-security-matrix.js','windows-r313-security-matrix.js','windows-r316-startup-acceptance.js','windows-r318-hard-exit-acceptance.js','windows-r39-broker-smoke.js','windows-r39-acceptance.js','windows-r39-manager-acceptance.js'])assert.equal(workflow.includes(runtimeGate),false,runtimeGate);assert.match(workflow,/Windows replacement fail-closed policy acceptance/);
 ...
 assert.match(workflow,/Windows replacement fail-closed policy acceptance[\s\S]{0,100}timeout-minutes: 5/);assert.match(workflow,/R39 packaged renderer and real-main lifecycle E2E[\s\S]{0,100}timeout-minutes: 11/);assert.doesNotMatch(workflow,/r39-native-\*\.log/);assert.match(workflow,/growthmap-r39-renderer\.png/);

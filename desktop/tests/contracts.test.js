...
test('commercial clipboard IPC is guarded and renderer cannot supply text',()=>{const page=fs.readFileSync(path.resolve(__dirname,'../../src/frontend/src/app/page.tsx'),'utf8');assert.match(main,/commercial:copy-base-payee.*guard\(e\).*clipboard\.writeText\(commercialConfig\.basePayee\)/s);assert.match(preload,/copyBasePayee:\(\)=>ipcRenderer\.invoke\('commercial:copy-base-payee'\)/);assert.doesNotMatch(preload,/copyBasePayee:[^\n]*=>[^\n]*,[^\n]*commercial:copy-base-payee/);assert.doesNotMatch(page,/navigator\.clipboard/);assert.match(page,/await window\.growthmapDesktop\.purchase\.copyBasePayee\(\)/);assert.match(page,/copyPayeeStatus/);assert.match(page,/catch \(e: unknown\)/);});
test('purchase IPC accepts exactly one rail and preload forwards no renderer context',()=>{assert.match(main,/purchase:open'.*\.\.\.args.*purchaseTargetForArgs\(args,commercialConfig,validPaypalUrl\)/s);assert.match(preload,/open:rail=>ipcRenderer\.invoke\('purchase:open',rail\)/);assert.doesNotMatch(preload,/open:\(rail,context\)|orderContext|recoveryContext/);assert.doesNotMatch(main,/paypal'.*new URL\(url\)\.origin/);assert.doesNotMatch(main,/PAYMENT-SIGNATURE|walletSignature|orderContext|recoveryContext/);});
...
test('production package verifier uses a CRLF fixture and checks actual ASAR/resource boundaries',()=>{
 ...
 for(const field of ['licenseIssuer','supportEmail','supportUrl','paymentApiOrigin','purchasePortalOrigin','purchasePortalUrl','baseNetwork','baseUsdc','basePayee','earlyLimit','earlyPriceMicros','regularPriceMicros','paypalUrl'])assert.match(script,new RegExp(`'${field}'`));

...
test('packaged E2E proves foreign import converges to exact fixture health before backup and restart',()=>{
 const source=fs.readFileSync(path.join(__dirname,'../scripts/renderer-e2e.js'),'utf8');
 for(const token of ['import-success-health','import-transaction-cleaned','after-replacement-cleanup','replacement-cleanup-pending','growthmapImportDocumentNonce','beforeImport','/api/health/deep','/api/projects/fixture','Desktop Fixture','database-backup','restart-free','assertRestartCredential'])assert.ok(source.includes(token),token);
 ...
 assert.match(source,/phaseWait\(run,'import-transaction-cleaned','after-replacement-cleanup',120000,\{failToken:'replacement-cleanup-pending'\}\)[\s\S]{0,200}credentialProofResult=await credentialProof/);
 assert.match(source,/if\(failToken&&lines\.some\(line=>line\.endsWith\(` \$\{failToken\}`\)\)\)throw Error/);
 const manager=fs.readFileSync(path.join(__dirname,'..','database-manager.js'),'utf8'),cleanup=manager.indexOf("report('after-replacement-cleanup')"),recover=manager.lastIndexOf('await recoverReplacement',cleanup),result=manager.indexOf('return {ok:true,installed:result.installed,cleanup}',cleanup),pending=manager.indexOf("report('replacement-cleanup-pending')",cleanup);assert.ok(recover>=0&&recover<cleanup&&cleanup<pending&&pending<result,'E2E cleanup phases must follow awaited terminal recovery, distinguish pending cleanup, and precede replacement resolution');

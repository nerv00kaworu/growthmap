...
test('packaged E2E proves foreign import converges to exact fixture health before backup and restart',()=>{
 const source=fs.readFileSync(path.join(__dirname,'../scripts/renderer-e2e.js'),'utf8');
 for(const token of ['import-success-health','import-transaction-cleaned','replacement-cleanup-complete','replacement-cleanup-pending','growthmapImportDocumentNonce','beforeImport','/api/health/deep','/api/projects/fixture','Desktop Fixture','database-backup','restart-free','assertRestartCredential'])assert.ok(source.includes(token),token);
 ...
 assert.match(source,/phaseWait\(run,'import-transaction-cleaned','replacement-cleanup-complete',120000,\{failToken:'replacement-cleanup-pending'\}\)[\s\S]{0,200}credentialProofResult=await credentialProof/);
 assert.match(source,/if\(failToken&&lines\.some\(line=>line\.endsWith\(` \$\{failToken\}`\)\)\)throw Error[\s\S]{0,300}if\(lines\.some\(line=>line\.endsWith\(` \$\{token\}`\)\)\)/);
 const manager=fs.readFileSync(path.join(__dirname,'..','database-manager.js'),'utf8'),cleanup=manager.indexOf("report('after-replacement-cleanup')"),recover=manager.lastIndexOf('await recoverReplacement',cleanup),result=manager.indexOf('return {ok:true,installed:result.installed,cleanup}',cleanup),pending=manager.indexOf("report('replacement-cleanup-pending')",cleanup),complete=manager.indexOf("report('replacement-cleanup-complete')",cleanup);assert.ok(recover>=0&&recover<cleanup&&cleanup<pending&&pending<complete&&complete<result,'E2E cleanup phases must follow awaited terminal recovery and report exactly one terminal outcome before replacement resolution');
 for(const token of ['before-replacement-cleanup','replacement-cleanup-quarantined','replacement-cleanup-durable','replacement-cleanup-terminal-marker'])assert.ok(manager.includes(`report('${token}')`),token);

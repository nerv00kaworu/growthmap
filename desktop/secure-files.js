...
function applyWindowsPrivateAcl(target,options={}){
 ...
 if(r.error){const code=/^[A-Z0-9_]{1,32}$/.test(String(r.error.code||''))?r.error.code:'unknown';throw Error(`Windows private secret path initialization failed stage=spawn-${code}`)}
 if(r.status!==0){const status=Number.isInteger(r.status)&&r.status>=0&&r.status<=255?r.status:'unknown',signal=/^[A-Z0-9_]{1,32}$/.test(String(r.signal||''))?r.signal:'none';throw Error(`Windows private secret path initialization failed stage=exit-${status}-${signal}`)}
}

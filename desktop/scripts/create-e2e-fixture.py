...
CREATE TABLE agent_proposals(id VARCHAR(36) PRIMARY KEY,grant_id VARCHAR(36) NOT NULL REFERENCES agent_grants(id) ON DELETE CASCADE,project_id VARCHAR(36) NOT NULL REFERENCES projects(id) ON DELETE CASCADE,target_node_id VARCHAR(36) REFERENCES nodes(id) ON DELETE SET NULL,title VARCHAR(200) NOT NULL,rationale TEXT DEFAULT '',operations JSON NOT NULL,expected_project_revision INTEGER NOT NULL,status VARCHAR(20) NOT NULL DEFAULT 'pending',review_note TEXT DEFAULT '',reviewed_at DATETIME,created_at DATETIME);
CREATE TABLE agent_events(id VARCHAR(36) PRIMARY KEY,grant_id VARCHAR(36) NOT NULL REFERENCES agent_grants(id) ON DELETE CASCADE,project_id VARCHAR(36) NOT NULL REFERENCES projects(id) ON DELETE CASCADE,target_node_id VARCHAR(36) REFERENCES nodes(id) ON DELETE SET NULL,event_type VARCHAR(30) NOT NULL,message TEXT NOT NULL,payload JSON NOT NULL DEFAULT '{}',created_at DATETIME);
...
CREATE INDEX ix_agent_proposals_grant_id ON agent_proposals(grant_id);
CREATE INDEX ix_agent_proposals_project_id ON agent_proposals(project_id);
CREATE INDEX ix_agent_events_grant_id ON agent_events(grant_id);
CREATE INDEX ix_agent_events_project_id ON agent_events(project_id);

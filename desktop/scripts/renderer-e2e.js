...
async function phaseWait(run,name,token,timeout=30000){console.log(`E2E stage start: ${name}`);const deadline=Date.now()+timeout;while(Date.now()<deadline){if(run.child.exitCode!==null)throw Error(`${name}: app exited; child=${run.output().slice(-1500)}`);let phases='';try{phases=fs.readFileSync(run.diagnosticPath,'utf8');}catch(error){if(error.code!=='ENOENT')throw error;}if(phases.split(/\r?\n/).some(line=>line.endsWith(` ${token}`))){console.log(`E2E stage pass: ${name}`);return;}await sleep(250);}throw Error(`${name}: timeout; phases=${JSON.stringify(fs.existsSync(run.diagnosticPath)?fs.readFileSync(run.diagnosticPath,'utf8').slice(-2000):'missing')}; child=${run.output().slice(-1500)}`);}

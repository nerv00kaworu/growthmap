...
async function phaseWait(run,name,token,timeout=30000,{failToken=null}={}){console.log(`E2E stage start: ${name}`);const deadline=Date.now()+timeout;while(Date.now()<deadline){if(run.child.exitCode!==null)throw Error(`${name}: app exited; child=${run.output().slice(-1500)}`);let phases='';try{phases=fs.readFileSync(run.diagnosticPath,'utf8');}catch(error){if(error.code!=='ENOENT')throw error;}const lines=phases.split(/\r?\n/);if(failToken&&lines.some(line=>line.endsWith(` ${failToken}`)))throw Error(`${name}: replacement cleanup remained pending; phases=${JSON.stringify(phases.slice(-2000))}; child=${run.output().slice(-1500)}`);if(lines.some(line=>line.endsWith(` ${token}`))){console.log(`E2E stage pass: ${name}`);return;}await sleep(250);}throw Error(`${name}: timeout; phases=${JSON.stringify(fs.existsSync(run.diagnosticPath)?fs.readFileSync(run.diagnosticPath,'utf8').slice(-2000):'missing')}; child=${run.output().slice(-1500)}`);}
...
(async()=>{
 ...
 try{
  ...
  const beforeImport=await run.page.evaluate(async()=>{const health=await fetch('/api/health/deep',{credentials:'same-origin'});document.documentElement.dataset.growthmapImportDocumentNonce='pre-import-document';return {health:health.status,title:Boolean(document.querySelector('[data-testid="growthmap-title"]'))}});if(beforeImport.health!==200||!beforeImport.title)throw Error(`import precondition failed: ${JSON.stringify(beforeImport)}`);
  ...
  await phaseWait(run,'import-transaction-cleaned','replacement-cleanup-complete',120000,{failToken:'replacement-cleanup-pending'});
  ...
  console.log(`Packaged normal-startup, foreign import, backup, credential restart and fail-closed lifecycle E2E passed; screenshot=${screenshot}`);

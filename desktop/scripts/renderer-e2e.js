...
async function launch(userData,fixture,profileMode,{injectHydrationFailure=false,expectStartupFailure=false}={}){
 ...
 let output='',directWebSocket=null,browser,pids=[child.pid];const capture=x=>{output+=x;directWebSocket=parseDevToolsWebSocket(output,debugPort);};child.stdout.on('data',capture);child.stderr.on('data',capture);
 try{

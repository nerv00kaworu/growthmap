...
async function launch(userData,fixture){const debugPort=await port();const child=spawn(executable,[`--user-data-dir=${userData}`],{env:{...process.env,CI:'true',GROWTHMAP_DESKTOP_E2E:'1',GROWTHMAP_E2E_DEBUG_PORT:String(debugPort),GROWTHMAP_E2E_IMPORT_PATH:fixture},stdio:['ignore','pipe','pipe'],windowsHide:true});let output='';child.stdout.on('data',x=>output+=x);child.stderr.on('data',x=>output+=x);let browser;const deadline=Date.now()+120000;while(Date.now()<deadline){if(child.exitCode!==null)throw Error(`app exited early: ${output.slice(-1500)}`);try{browser=await chromium.connectOverCDP(`http://127.0.0.1:${debugPort}`,{timeout:2000});break;}catch{await sleep(250);}}if(!browser)throw Error('CDP timeout');let page;while(Date.now()<deadline&&!page){page=browser.contexts().flatMap(c=>c.pages()).find(p=>p.url().startsWith('http://127.0.0.1:'));if(!page)await sleep(200);}if(!page)throw Error(`renderer page missing; child=${output.slice(-1500)}`);const run={child,browser,page,tree:descendants(child.pid),output:()=>output};await stageWait(run,'renderer-ready',()=>Boolean(document.querySelector('[data-testid="growthmap-title"]')&&document.querySelector('[data-testid="desktop-settings-button"]')),90000);return run;}
...
(async()=>{
 ...
 const userData=fs.mkdtempSync(path.join(os.tmpdir(),'growthmap-e2e-')),fixture=path.join(userData,'fixture.sqlite');
 ...
 }finally{if(run?.child.exitCode===null)spawnSync('taskkill',['/PID',String(run.child.pid),'/T','/F'],{windowsHide:true});}

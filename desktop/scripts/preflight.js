...
if(process.env.GROWTHMAP_COMMERCIAL_RELEASE==='1'){
 ...
 const purchaseOrigin=commercial.purchasePortalOrigin||'',purchaseUrl=commercial.purchasePortalUrl||'',activationOrigin=commercial.activationApiOrigin||'';
 const signed=platform==='win32'&&Boolean(process.env.WIN_CSC_LINK&&process.env.WIN_CSC_KEY_PASSWORD);
 const publisherOk=identity.status==='APPROVED'&&typeof identity.windowsPublisher==='string'&&!/REPLACE|EXAMPLE|TBD/i.test(identity.windowsPublisher)&&pkg.build.win.signtoolOptions?.publisherName===identity.windowsPublisher;
 const majorOk=Number(String(pkg.version).split('.')[0])===identity.productMajor&&identity.productMajor===1;
 let endpointsOk=false,commercialOk=false;try{const {validateUpdateUrl,checkoutAllowed,canonicalOrigin}=require('../update-policy');const {strictCanonicalPublicHttpsOrigin}=require('../purchase-portal');const commercialModule=require('../commercial-config');validateUpdateUrl(updateUrl,{production:true});const portal=new URL(purchaseOrigin);endpointsOk=portal.protocol==='https:'&&!/example\.(com|invalid)|localhost|127\.0\.0\.1/i.test(portal.href)&&checkoutAllowed(purchaseUrl,purchaseOrigin)&&strictCanonicalPublicHttpsOrigin(activationOrigin)===activationOrigin&&activationOrigin!==purchaseOrigin&&new URL(updateUrl).origin===canonicalOrigin(commercial.updateOrigin);const expected=commercialModule.EXACT;commercialOk=Object.keys(commercial).sort().join('|')===expected.sort().join('|')&&commercial.publisherStatus==='APPROVED'&&commercial.publisher===identity.windowsPublisher&&commercial.productMajor===identity.productMajor&&commercial.licensePublicKeySha256===require('node:crypto').createHash('sha256').update(fs.readFileSync(publicKey)).digest('hex')&&!/REPLACE|EXAMPLE|TBD|UNAPPROVED/i.test(JSON.stringify(commercial));}catch{}

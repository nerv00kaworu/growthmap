...
if(process.env.GROWTHMAP_COMMERCIAL_RELEASE==='1'){
 ...
 let endpointsOk=false,commercialOk=false;try{const {validateUpdateUrl,checkoutAllowed,canonicalOrigin}=require('../update-policy');validateUpdateUrl(updateUrl,{production:true});const portal=new URL(purchaseOrigin);endpointsOk=portal.protocol==='https:'&&!/example\.(com|invalid)|localhost|127\.0\.0\.1/i.test(portal.href)&&checkoutAllowed(purchaseUrl,purchaseOrigin)&&checkoutAllowed(paypalUrl,new URL(paypalUrl).origin)&&canonicalOrigin(commercial.paymentApiOrigin)===commercial.paymentApiOrigin&&new URL(updateUrl).origin===canonicalOrigin(commercial.updateOrigin);const expected=require('../commercial-config').EXACT;commercialOk=Object.keys(commercial).sort().join('|')===expected.sort().join('|')&&commercial.publisherStatus==='APPROVED'&&commercial.publisher===identity.windowsPublisher&&commercial.productMajor===identity.productMajor&&commercial.licensePublicKeySha256===require('node:crypto').createHash('sha256').update(fs.readFileSync(publicKey)).digest('hex')&&!/REPLACE|EXAMPLE|TBD|UNAPPROVED/i.test(JSON.stringify(commercial));}catch{}

...
`uninitialized -> free` only after the first successful desktop sidecar launch. Free is permanent and permits exactly one concurrent active project. Archive or delete frees the seat; restore and import are subject to the same atomic seat check. Legacy valid trial-state evidence of any age is interpreted as Free. A valid signed major-1 entitlement transitions any local state to `paid`; invalid, expired, refunded/revoked (after a truthful check-in), corrupt, or wrong-major documents do not. Paid major 1 is perpetual unless its signed document explicitly has an expiry (nonstandard grant) or a later check-in replaces it with a signed revocation assertion. Failure never deletes data.

States:
- **free**: all normal product mutations; exactly one active project.
...
| Capability | Free | Paid matching major | Extraction |
|---|---:|---:|---:|
...
| create/update/delete/archive/restore project | allow (1 active) | allow | **deny** |
...
Implementation uses a centralized fail-closed rule in desktop mode: GET/HEAD/OPTIONS plus license import and idempotent explicit Free initialization are the entitlement-extraction HTTP allowlist. Every other POST/PUT/PATCH/DELETE is denied before route execution. Desktop IPC permits backup/status/list/reveal; entitlement extraction denies import/restore. A distinct pending-update recovery lock denies import/product mutation but permits managed backup restore and reveal. Authoring Web mode does not mount this restriction.

## Clock, offline, corruption, concurrency

Free provenance timestamps are UTC and atomically persisted under Electron `userData`; `started_at` never resets and is not an expiry. `last_seen_at` advances monotonically only through an explicitly authenticated writable lifecycle checkpoint; entitlement GET/peek and every extraction/recovery GET are side-effect free. A wall-clock rollback over five minutes, future start, malformed/unknown fields, unreadable state, or write failure fails closed to extraction. Small clock jitter is tolerated. No network is required for Free or paid use. Extraction is always available even when state/license verification fails. Project seat allocation is guarded by a process lock plus a fresh database count so concurrent create/restore/import activation cannot exceed one.

...
Shared-truth status: `create_node`, `update_node`, `create_edge`, `create_content_block`, `update_content_block`, `delete_content_block`, and `create_branch` now use shared canonical primitives across GUI and Agent entry points. `update_edge`, `delete_edge`, and full graph/context read services remain deferred to later slices.
...
canvas position remain outside Agent Port. Create-edge, create-content-block,
update-content-block, and delete-content-block are shared; update/delete edge and
full graph/context read services remain deferred.

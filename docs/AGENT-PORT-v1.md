...
Shared-truth status: `create_node`, `update_node`, `create_edge`, `create_content_block`, `update_content_block`, and `create_branch` now use shared canonical primitives across GUI and Agent entry points. `update_edge`, `delete_edge`, `delete_content_block`, and full graph/context read services remain deferred to later slices.

...
Shared-truth status: `create_node`, `update_node`, `create_edge`, `update_edge`, `create_content_block`, `update_content_block`, `delete_content_block`, and `create_branch` now use shared canonical primitives across GUI and Agent entry points. P0 `update_edge` is restricted to weight/note on pre-existing non-`child_of` relations with persisted two-endpoint scope and union CAS. `delete_edge` and full graph/context read services remain deferred to later slices.

...
A candidate `OfficialX402Facilitator` adapter for the pinned x402 2.17 contract exists alongside the deterministic isolated-test `MockFacilitator`. It deliberately does not claim production readiness: production credentials, a real recipient/endpoint, authenticated receipt/finality reconciliation, deployment review and live smoke evidence remain blockers. Recipient placeholders fail startup. Human wallets open an external HTTPS portal/WalletConnect seam; the desktop never receives keys or fabricates x402 headers. Packaged configuration separately pins `paymentApiOrigin` and `purchasePortalOrigin`, requires each configured origin to already be a strict canonical public HTTPS origin using a DNS hostname (all IPv4 and IPv6 literals are rejected, including globally routed and special-purpose ranges), rejects trailing-dot hostnames, requires the authorities to differ, and fails startup/preflight when either authority or the exact query-free/hash-free portal URL is absent or malformed. Electron purchase IPC accepts exactly one rail and never accepts a renderer context object, identifier, token, credential, signature, wallet artifact, query, fragment, or destination URL. x402 opens only the exact packaged portal target; PayPal opens only the exact approved URL. Wallet/private-key operations and `PAYMENT-SIGNATURE` creation remain portal-only.

Fragments are **not used by the current desktop seam**. If future authoritative portal capability issuance introduces fragment transport, acceptance is BLOCKED_EXTERNAL until it proves one-purpose, short-lived, one-use issuance and portal consumption that clears the fragment with `history.replaceState` before any third-party script or analytics runs. Fragment values remain visible to the address bar, history, session restore, destination scripts, browser extensions, screenshots, telemetry/crash reports, and redirects/client-side navigation; they must never be persisted or logged. Fragment omission from the initial HTTP request, Referer, and ordinary access logs is leakage reduction, not secrecy. Redirect policy must also be independently reviewed.
...
## Production source-candidate vs external gates

This repository is a **source candidate**, not publication evidence. A committed source gate means fresh Security/QA has reviewed the exact diff and tests. A genuine Windows publication gate additionally requires an authorized runner to build the effective signed configuration, inspect the actual ASAR/resources, verify Authenticode signer/status/timestamp/chain, installer and portable hashes, updater metadata/hash/signature, and run packaged purchase→activation E2E. Mainnet live approval is a separate explicit authorization for a stated minimum amount; no local/mock transaction or SDK settle response substitutes for it.

Deployment order is fail closed: (1) review/pin public origins, payee and packaged verification key; (2) provision external signer/facilitator/reconciler providers without exposing secret bytes to repository, desktop, DB, responses, or logs; (3) migrate and restore-test payment and Authority DBs independently; (4) start Authority and confirm key-id/public-key binding; (5) start payment API and outbox workers; (6) deploy portal; (7) run genuine Windows gate; (8) only after separate approval, run the minimum-value live smoke. Roll back application binaries/config before accepting new orders, but never roll back DB/evidence/checkpoint/key-generation state; quarantine conflicts for manual review.

Monitor ambiguous intents, finality age, reconciler authentication failures, evidence conflicts/replays, outbox lease age/retry/quarantine, Authority inbox conflicts, activation proof failures, revocation lag, checkpoint mismatch, backup/restore age, signer key-id/generation mismatch, and Windows publication attestations. Alert rather than blind-retry unknown settlement. Back up each DB with its own authenticated checkpoint/monotonic provider state and test restoration in isolation.

## Production blockers

Official x402/CDP SDK facilitator; production Base recipient; secure HSM/KMS/offline Ed25519 provider and matching packaged public key; real HTTPS payment API + purchase portal/WalletConnect; authoritative future portal capability issuance/verification (one-purpose, short-lived, one-use, and cleared before scripts if transported in a fragment); PayPal merchant/invoice URL and reviewed admin operations; Argon2id/session/CSRF/rate limiting; email/recovery delivery; monitoring, reconciliation, backups and privacy retention; signed revocation/check-in; legal approval; Windows signed package/E2E. Placeholders intentionally fail commercial preflight.
...
## Production public-input candidate (2026-08-01)

Approved public inputs are now recorded in strict desktop/server public configs with parity tests: Base payee `0x81d30e175a22c1c2f78b3db6fc0600a6e1cb3591`, PayPal payment URL, issuer display `Growthmap`, and owner Email/X support contacts. They are not credentials and do not complete production. Remaining gates are a real payment API origin and purchase portal, facilitator credentials, authenticated finality reconciler, production public key plus key ceremony, update endpoint, and deployment validation. The candidate remains fail closed.

Public payment coordinates are generated deterministically from `config/commercial-public-config.json`; the server validates a shipped digest and safe regular-file properties. Nonproduction environments reject the approved live recipient unless `GROWTHMAP_ALLOW_LIVE_PAYMENT_RECIPIENT=1` is explicitly set.

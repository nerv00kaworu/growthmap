...
def create_gift_admin_app(*, authority: SignedAuthorityHTTPAdapter, access: AccessJWTVerifier,
    ...
    async def page(request:Request):
        ...
        script="""const csrf=document.querySelector('meta[name=csrf]').content;const out=document.getElementById('result');async function call(path,body){out.textContent='處理中…';const r=await fetch(path,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf},body:JSON.stringify(body)});const j=await r.json();out.textContent=JSON.stringify(j,null,2);if(!r.ok)throw new Error('request failed');return j}document.getElementById('create').onsubmit=async e=>{e.preventDefault();const n=Number(document.getElementById('count').value);await call('/v1/admin/gifts/create',{count:n,edition:'personal',major_version:1,seat_limit:2,expires_at:null,check_in_days:30});};document.querySelectorAll('button[data-action]').forEach(b=>b.onclick=async()=>{const action=b.dataset.action,id=b.dataset.id;if(action==='revoke'&&!confirm('確定撤銷這組親友授權？'))return;await call('/v1/admin/gifts/'+action,{gift_id:id});});"""
        response=HTMLResponse(f"<!doctype html><html lang='zh-Hant'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><meta name=csrf content='{html.escape(identity['csrf'])}'><title>GrowthMap Gift Admin</title><style>body{{font:16px system-ui;background:#0b1020;color:#eef2ff;margin:0;padding:32px}}main{{max-width:1100px;margin:auto}}section,table{{background:#151d35;border:1px solid #2b385d;border-radius:14px;padding:18px;margin:18px 0}}table{{width:100%;border-collapse:collapse}}th,td{{text-align:left;padding:10px;border-bottom:1px solid #2b385d}}button,input{{font:inherit;padding:8px 12px}}button{{cursor:pointer}}pre{{white-space:pre-wrap;overflow-wrap:anywhere;background:#070b15;padding:16px;border-radius:10px}}</style></head><body><main><h1>GrowthMap 親友版管理</h1><p>預設 Personal v1、永久、2 台裝置。金鑰只會在建立或恢復時顯示一次。</p><section><form id='create'><label>生成數量（1–20） <input id='count' type='number' min='1' max='20' value='1' required></label> <button type='submit'>生成親友金鑰</button></form></section><table><thead><tr><th>Gift ID</th><th>狀態</th><th>裝置上限</th><th>建立時間</th><th>操作</th></tr></thead><tbody>{rows}</tbody></table><h2>單次結果</h2><pre id='result'>尚未執行操作。</pre></main><script>{script}</script></body></html>")
        if issued:response.set_cookie("gm_gift_admin",issued,max_age=sessions.ttl,secure=True,httponly=True,samesite="strict",path="/")
        return response

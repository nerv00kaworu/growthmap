...
from db.schema_contract import CURRENT_USER_VERSION, CANONICAL_TABLES, REQUIRED_TABLE_COLUMNS, OBJECT_METADATA, ROW_REQUIRED_NON_NULL, authority_critical_fk_violations, evaluate_snapshot, provider_selection_contract_problem
...
CORE_TABLES=set(CANONICAL_TABLES)
...
ALLOWED_INDEX_TABLES.update({name: meta["owner"] for name,meta in OBJECT_METADATA.items() if meta["kind"]=="index"})
ALLOWED_TRIGGER_TABLES.update({name: meta["owner"] for name,meta in OBJECT_METADATA.items() if meta["kind"]=="trigger"})
...
REQUIRED=REQUIRED_TABLE_COLUMNS
...
def _canonical_snapshot(connection):
 tables={row[0] for row in connection.execute("SELECT name FROM sqlite_schema WHERE type='table'")}
 infos={table:connection.execute(f'PRAGMA table_info("{table}")').fetchall() for table in tables}
 objects={row[1]:(row[0],row[2]) for row in connection.execute("SELECT type,name,sql FROM sqlite_schema WHERE type IN ('index','trigger')")}
 nulls=[]
 for table,columns in ROW_REQUIRED_NON_NULL.items():
  present={row[1] for row in infos.get(table,())}
  for column in columns:
   if column in present and connection.execute(f'SELECT 1 FROM "{table}" WHERE "{column}" IS NULL LIMIT 1').fetchone():nulls.append((table,column))
 selection="provider_selection" in tables
 fk_rows=connection.execute("PRAGMA foreign_key_check").fetchall()
 return {"version":int(connection.execute("PRAGMA user_version").fetchone()[0]),"tables":tables,"tableInfo":infos,"objects":objects,
  "nullViolations":nulls,"providerSelectionForeignKeys":connection.execute("PRAGMA foreign_key_list('provider_selection')").fetchall() if selection else (),
  "providerSelectionSql":connection.execute("SELECT sql FROM sqlite_schema WHERE type='table' AND name='provider_selection'").fetchone()[0] if selection else None,

...
import pytest
ROOT=Path(__file__).parents[2];R21=ROOT.parents[0]/'reports/phase2-authority-production-readiness-20260808/vps-staging-r21';DEP=R21/'deploy'
def module(p,n):
...
def test_release_identity_strict_and_old_process_rejected(monkeypatch,tmp_path):
 ...
 a=module(DEP/'acceptance.py','r21accept');cfg=tmp_path/'composition.json';auth=tmp_path/'authority-production.json';auth.write_text(json.dumps({'reviewed_public_key_sha256':'1'*64}));cfg.write_text(json.dumps({'authority_config_path':str(auth),'listen':{'host':'127.0.0.1','port':8320}}))
...
def test_unit_exact_parity_and_stable_security_config():
 u=(DEP/'growthmap-authority-staging-r21.service.template').read_text();p=(R21/'provision_staging.py').read_text()
 assert 'python -I -B -m licensing.authority_composition --fixed-config /etc/growthmap-authority-staging-r21/production-composition.json --expected-service-uid @SERVICE_UID@ --release-digest @RELEASE_DIGEST@' in u
...
def test_registry_binds_authority_pin_and_venv(tmp_path):
 r=module(DEP/'registry.py','r21registry');release=tmp_path/('a'*64);release.mkdir();
 for n,v in [('runtime.tar',b'x'),('source-files.sha256',b'y'),('venv-manifest.json',b'z')]: (release/n).write_bytes(v)
 unit=tmp_path/'growthmap-authority-staging-r21.service';unit.write_text('u');auth=tmp_path/'authority-production.json';auth.write_text(json.dumps({'reviewed_public_key_sha256':'b'*64}));cfg=tmp_path/'production-composition.json';cfg.write_text(json.dumps({'expected_uid':123,'listen':{'port':8320},'edge':{'public_key_sha256':'c'*64}}))
...
def test_transaction_fresh_and_existing_byte_metadata_restore(tmp_path):
 t=module(DEP/'transaction.py','r21tx');root=tmp_path/'state';snap=tmp_path/'snap';t.snapshot(root,snap);root.mkdir();(root/'new').write_bytes(b'n');t.restore(root,snap);assert not root.exists()
...
def test_transaction_rejects_hardlink_and_external_symlink(tmp_path):
 t=module(DEP/'transaction.py','r21txbad');root=tmp_path/'state';root.mkdir();a=root/'a';a.write_text('x');os.link(a,root/'b')
...
def test_real_venv_copies_inventory_structural_policy(tmp_path):
 v=module(DEP/'venv_manifest.py','r21venvreal');root=tmp_path/'venv';subprocess.run([sys.executable,'-B','-m','venv','--copies',str(root)],check=True,env={**os.environ,'PYTHONDONTWRITEBYTECODE':'1'})
...
def test_venv_verifier_rejects_addition_symlink_hardlink_and_pyc(tmp_path):
 v=module(DEP/'venv_manifest.py','r21venv');root=tmp_path/'v';(root/'bin').mkdir(parents=True);(root/'bin/python3').write_bytes(b'elf');(root/'a').write_bytes(b'a');assert any(x['path']=='a' for x in v.inventory(root));(root/'x.pyc').write_bytes(b'x')
...
def _fake_apply_env(tmp_path,monkeypatch,fail='',baseline='present',initial='active enabled'):
 import shutil
 rail=tmp_path/'rail';shutil.copytree(R21,rail);root=tmp_path/'root';(root/'opt').mkdir(parents=True);(root/'etc/systemd/system').mkdir(parents=True);
 if baseline=='present':
  (root/'etc/systemd/system/growthmap-authority-staging-r21.service').write_text('baseline-unit');(root/'var/lib/growthmap-authority-staging-r21-transactions').mkdir(parents=True);(root/'var/lib/growthmap-authority-staging-r21-transactions').chmod(0o700)
 for script in (rail/'deploy/deploy.sh',rail/'deploy/rollback.sh'):
  text=script.read_text()
  bind=tmp_path/'bin'
  paths=[('/var/lib/growthmap-authority-staging-r21-transactions',str(root/'var/lib/growthmap-authority-staging-r21-transactions')),('/opt/growthmap-authority-staging-r21',str(root/'opt/growthmap-authority-staging-r21')),('/var/lib/growthmap-authority-staging-r21',str(root/'var/lib/growthmap-authority-staging-r21')),('/etc/growthmap-authority-staging-r21',str(root/'etc/growthmap-authority-staging-r21')),('/etc/systemd/system',str(root/'etc/systemd/system'))]
  for i,(canonical,adapted) in enumerate(paths):
   marker=f'__R21_ADAPTER_PATH_{i}__';assert marker not in text and canonical in text;text=text.replace(canonical,marker)
  for i,(_canonical,adapted) in enumerate(paths):text=text.replace(f'__R21_ADAPTER_PATH_{i}__',adapted)
  ...
  protected=('--tar','-m pip install','-Hln','source-files.sha256','R21_TRANSACTION_INJECT')
...
''');dispatcher.write_text(dispatcher.read_text().replace('#!/usr/bin/env python3','#!'+sys.executable));dispatcher.chmod(0o755)
 ...
 env={**os.environ,'PATH':str(bind)+':'+os.environ['PATH'],'FAKE_LOG':str(log),'SYSTEMCTL_STATE':str(state),'FAIL_STAGE':fail,'FAKE_PYTHON3':str(bind/'python3'),'RAIL':str(rail),'UNIT_PATH':str(root/'etc/systemd/system/growthmap-authority-staging-r21.service')}
...
def test_entire_deploy_fresh_failure_restores_true_prestate(tmp_path,stage):
 rail,root,env,state,log=_fake_apply_env(tmp_path,None,stage);before=_tree(root);r=subprocess.run(['/bin/sh',str(rail/'deploy/deploy.sh'),'--apply'],env=env);assert r.returncode!=0 and _tree(root)==before and set(state.read_text().split())=={'active','enabled'};assert not any('.incoming-' in x[0] for x in _tree(root))
...
def test_entire_deploy_upgrade_and_rollback_active_lifecycle(tmp_path):
 ...
 current=root/'opt/growthmap-authority-staging-r21/current';assert os.readlink(current)=='releases/'+b
 trace=log.read_text()
 ...
 assert subprocess.run(['/bin/sh',str(rail/'deploy/rollback.sh'),'--apply',a],env=env).returncode==0;assert os.readlink(current)=='releases/'+a and 'restart growthmap-authority-staging-r21.service' in log.read_text()
...
def test_first_install_absent_success_and_exact_systemd_semantics(tmp_path):
 ...
 assert (root/'etc/systemd/system/growthmap-authority-staging-r21.service').is_file()
 assert set(state.read_text().split())=={'active','enabled'}
 trace=log.read_text();assert 'systemctl show growthmap-authority-staging-r21.service --property=LoadState --value' in trace
 assert trace.index('systemctl show ')<trace.index('install -d -o 0 -g 0 -m 0700')
...
def test_first_install_failure_restores_exact_absence(tmp_path,stage):
 rail,root,env,state,log=_fake_apply_env(tmp_path,None,fail=stage,baseline='absent',initial='inactive');unit=root/'etc/systemd/system/growthmap-authority-staging-r21.service'
 r=subprocess.run(['/bin/sh',str(rail/'deploy/deploy.sh'),'--apply'],env=env,capture_output=True,text=True)
...
def test_mixed_or_ambiguous_systemd_state_rejected_before_journal(tmp_path,unit_present,initial,expected):
 ...
 if not unit_present:assert not (root/'var/lib/growthmap-authority-staging-r21-transactions').exists()

def test_production_rejects_root_overrides():
 for script,args in [(DEP/'deploy.sh',[]),(DEP/'rollback.sh',['--apply','a'*64])]: 
  for key in ('R21_ROOT','R15_ROOT','GROWTHMAP_ROOT'):
...
def test_deploy_pre_snapshot_failures_restore_status_and_files(tmp_path,stage):
 ...
 r=subprocess.run(['/bin/sh',str(rail/'deploy/deploy.sh'),'--apply'],env=env)
 assert r.returncode!=0 and _tree(root)==before and set(state.read_text().split())=={'active','enabled'}
...
def test_sqlite_fail_closed_wal_and_partial_cleanup(tmp_path,monkeypatch):
 t=module(DEP/'transaction.py','r21txsqlite');bad=tmp_path/'bad.db';bad.write_bytes(b'not sqlite');snap=tmp_path/'snap'
...
def test_transaction_rejects_overlap_and_symlink_parent(tmp_path):
 t=module(DEP/'transaction.py','r21txcontain');a=(tmp_path/'a').absolute();a.mkdir()
...
def test_relocated_console_script_and_no_incoming_bytes(tmp_path):
 r=module(DEP/'relocate_venv.py','r21relocate');incoming=tmp_path/'.incoming-x';release=tmp_path/'release';v=incoming/'venv';(v/'bin').mkdir(parents=True);(release/'venv/bin').mkdir(parents=True)
...
def test_private_temp_never_touches_predictable_symlink(tmp_path):
 ...
 sentinel=tmp_path/'sentinel';sentinel.write_text('safe');predictable=tmp_path/'growthmap-authority-staging-r21.service.1';predictable.symlink_to(sentinel)
 assert sentinel.read_text()=='safe'


def _r21_multi(tmp_path):
 t=module(DEP/'transaction.py','r21txmulti');roots=[]
 for i in range(4):
...
def test_r21_all_root_restore_and_prevalidation(tmp_path):
 t,roots,snap,jr=_r21_multi(tmp_path);t.restore_all(snap,roots,jr)
 assert [(p/'v').read_text() for p in roots]==['B0','B1','B2','B3'] and not list(jr.iterdir())

def test_r21_root_boundary_failures_undo_to_exact_candidate(tmp_path,monkeypatch):
 for point in [*(f'before_backup_{i}' for i in range(4)),*(f'after_backup_{i}' for i in range(4)),*(f'before_install_{i}' for i in range(4)),*(f'after_install_{i}' for i in range(4))]:
  case=tmp_path/point;case.mkdir();t,roots,snap,jr=_r21_multi(case);monkeypatch.setenv('R21_TRANSACTION_INJECT',point)
  ...
  monkeypatch.delenv('R21_TRANSACTION_INJECT')

def test_r21_undo_failure_leaves_journal_and_recover_is_idempotent(tmp_path,monkeypatch):
 t,roots,snap,jr=_r21_multi(tmp_path);original=t._fail
 def injected(point):
...
def test_r21_metadata_tamper_matrix_is_before_mutation(tmp_path):
 mutators=[
 ...
 for n,mutate in enumerate(mutators):
  case=tmp_path/str(n);case.mkdir();t,roots,snap,jr=_r21_multi(case);before=[(p/'v').read_text() for p in roots];mp=snap/'0/meta.json';m=json.loads(mp.read_text());mutate(m);mp.write_text(json.dumps(m))
  with pytest.raises(Exception,match='transaction'): t.restore_all(snap,roots,jr)
  assert [(p/'v').read_text() for p in roots]==before

def test_r21_complete_extra_hardlink_and_symlink_tamper(tmp_path):
 for kind in ('complete','extra','hardlink','symlink'):
  case=tmp_path/kind;case.mkdir();t,roots,snap,jr=_r21_multi(case)
...
def test_r21_privileged_environment_and_absolute_python_contract():
 ...
 u=(DEP/'growthmap-authority-staging-r21.service.template').read_text()
 assert 'Environment=PATH=/usr/sbin:/usr/bin:/sbin:/bin' in u and 'Environment=PYTHONPATH=' in u

def test_r21_hostile_environment_cannot_run_path_shim(tmp_path):
...
PRODUCTION_FILES=tuple(sorted([*DEP.glob('*'),R21/'provision_staging.py']))
PRODUCTION_FILES=tuple(p for p in PRODUCTION_FILES if p.is_file())
SERVICE=DEP/'growthmap-authority-staging-r21.service.template'
SCANNER_PATH=R21/'evidence/semantic_scanner.py'

def _scanner():return module(SCANNER_PATH,'r21_semantic_scanner_'+os.urandom(4).hex())
def _audit_production(overrides=None,manifest_path=None):
 s=_scanner();return s.verify(overrides,manifest_path or s.MANIFEST)

def test_r21_canonical_scanner_manifest_and_evidence_same_code_path():
 s=_scanner();got=s.verify();log=json.loads((R21/'evidence/semantic-command-audit.log').read_text())
 assert log['status']=='PASS' and {k:log[k] for k in got}==got
 assert got['count']==len(got['records']) and got['production_files']==[p.relative_to(R21).as_posix() for p in PRODUCTION_FILES]
 assert set(got['by_class'])=={'shell_shebang','python_shebang','shell_command_token','service_execstart','python_path_literal'}
 assert got['by_class']['python_shebang']==8 and got['by_class']['python_path_literal']==2

def test_r21_scanner_manifest_count_and_evidence_divergence_fail_closed(tmp_path):
 ...
 ev=json.loads((R21/'evidence/semantic-command-audit.log').read_text());ev['count']+=1
 assert {k:ev[k] for k in s.scan()}!=s.scan()

@pytest.mark.parametrize('path',[p for p in PRODUCTION_FILES if p.suffix=='.py'])
def test_r21_every_python_shebang_mutation_rejected(path):
...
def test_r21_venv_python_path_mutations_rejected(path):
...
def test_r21_shell_absolute_command_and_all_r17_corruptions_rejected():
...
def test_r21_service_description_mutations_rejected(old):
 with pytest.raises(ValueError):_audit_production({SERVICE:SERVICE.read_text().replace('isolated staging R21','isolated staging '+old)})

@pytest.mark.parametrize('needle',(
 '/opt/growthmap-authority-staging-r21/current/venv/bin/python','/opt/growthmap-authority-staging-r21/current',
 'ReadOnlyPaths=/opt/growthmap-authority-staging-r21 /etc/growthmap-authority-staging-r21','ReadWritePaths=/var/lib/growthmap-authority-staging-r21',
 'growthmap-authority-staging-r21.service','/var/lib/growthmap-authority-staging-r21-transactions',
 '/opt/growthmap-authority-staging-r21','/etc/growthmap-authority-staging-r21','/var/lib/growthmap-authority-staging-r21'))
def test_r21_identity_and_service_field_mutations_rejected(needle):
 overrides={p:p.read_text().replace(needle,needle.replace('r21','r19')) for p in PRODUCTION_FILES}
 assert any(overrides[p]!=p.read_text() for p in PRODUCTION_FILES)
 with pytest.raises(ValueError):_audit_production(overrides)

def test_r21_real_plan_build_verify_wheels_and_recover(tmp_path):
 """Run canonical plan with real python/build/verify/checksums; adapt host identity and journal only."""
 import re,shutil
 work=tmp_path/'work';rail=work/'reports/phase2-authority-production-readiness-20260808/vps-staging-r21';rail.parent.mkdir(parents=True);shutil.copytree(R21,rail)
 ...
 text=text.replace('/var/lib/growthmap-authority-staging-r21-transactions',str(journal))

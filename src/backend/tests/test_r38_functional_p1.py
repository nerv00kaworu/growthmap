"""R38 installed-functional P1 regressions on isolated canonical API state."""
import asyncio
...
def create_block(client, project_id, node_id, block_type):
    response = client.post(f"/api/nodes/{node_id}/blocks", json={
        ...
        "content": {"title": block_type},
        # Deliberately repeat the renderer/default value; backend owns append order.
        "order_index": 0,
    })
    assert response.status_code == 201, response.text
    return response.json()
...
def test_block_creation_append_reorder_and_delete_are_contiguous():
    with TestClient(app) as client:
        project = client.post("/api/projects", json={"name": "ordered blocks"}).json()
        node_id = project["root_node_id"]
        types = ["paragraph", "bullet_list", "risk_note", "decision_log", "code", "resource", "markdown"]
        blocks = [create_block(client, project["id"], node_id, kind) for kind in types]
        assert [row["order_index"] for row in client.get(f"/api/nodes/{node_id}/blocks").json()] == list(range(7))
        moved = blocks[-1]
        response = client.patch(f"/api/blocks/{moved['id']}", json={
            "expected_project_revision": project_revision(client, project["id"]),
            "expected_node_revision": node_revision(client, node_id),
            "expected_revision": moved["revision"], "order_index": 2,
        })
        assert response.status_code == 200, response.text
        rows = client.get(f"/api/nodes/{node_id}/blocks").json()
        assert [row["order_index"] for row in rows] == list(range(7))
        victim = rows[3]
        response = client.request("DELETE", f"/api/blocks/{victim['id']}", json={
            "expected_project_revision": project_revision(client, project["id"]),
            "expected_node_revision": node_revision(client, node_id),
            "expected_revision": victim["revision"],
        })
        assert response.status_code == 204, response.text
        assert [row["order_index"] for row in client.get(f"/api/nodes/{node_id}/blocks").json()] == list(range(6))
    with TestClient(app) as restarted:
        assert [row["order_index"] for row in restarted.get(f"/api/nodes/{node_id}/blocks").json()] == list(range(6))
...
def test_legacy_block_order_is_canonicalized_with_sibling_revisions():
    with TestClient(app) as client:
        project = client.post("/api/projects", json={"name": "legacy order"}).json()
        node_id = project["root_node_id"]
        blocks = [create_block(client, project["id"], node_id, kind) for kind in ("note", "spec", "decision", "todo", "risk", "resource", "resource")]
        async def damage():
            async with async_session() as db:
                rows = list((await db.execute(select(ContentBlock).where(ContentBlock.node_id == node_id).order_by(ContentBlock.created_at))).scalars())
                for row, bad in zip(rows, (-4, 0, 0, 8, 8, 99, 2)):
                    row.order_index = bad
                await db.commit()
        asyncio.run(damage())
        before = {row["id"]: row["revision"] for row in client.get(f"/api/nodes/{node_id}/blocks").json()}
        created = create_block(client, project["id"], node_id, "note")
        rows = client.get(f"/api/nodes/{node_id}/blocks").json()
        assert [row["order_index"] for row in rows] == list(range(8))
        assert created["order_index"] == 7
        assert any(row["revision"] > before[row["id"]] for row in rows if row["id"] in before)
        moving = rows[-1]
        first = client.patch(f"/api/blocks/{moving['id']}", json={
            "expected_project_revision": project_revision(client, project["id"]),
            "expected_node_revision": node_revision(client, node_id),
            "expected_revision": moving["revision"], "order_index": -50,
        })
        assert first.status_code == 200, first.text
        stale = client.patch(f"/api/blocks/{moving['id']}", json={
            "expected_project_revision": project_revision(client, project["id"]),
            "expected_node_revision": node_revision(client, node_id),
            "expected_revision": moving["revision"], "order_index": 99,
        })
...
def test_mock_expand_all_modes_follow_authoritative_requested_mode():
    with TestClient(app) as client:
        project = client.post("/api/projects", json={"name": "mock modes"}).json()
        provider = client.post("/api/providers", json={"name": "mock", "provider_type": "mock"}).json()
        selection = client.put("/api/providers/selection", json={
            "provider_id": provider["id"], "expected_selection_revision": provider["selection_revision"],
        }).json()
        for mode in ("focused", "explore", "challenge"):
            response = client.post("/api/ai/expand", json={
                "node_id": project["root_node_id"], "provider_id": provider["id"],
                "provider_revision": provider["revision"], "selection_revision": selection["selection_revision"],
                "mode": mode, "count": 3, "locale": "en",
            })
            assert response.status_code == 200, response.text
            assert response.json()["context_used"]["mode"] == mode
            assert all(row["title"].startswith(f"[{mode}]") for row in response.json()["suggestions"])
...
def test_delete_rejects_diamond_parallel_and_different_depth_repeated_targets_without_any_change():
    ...
    for case in ("diamond", "parallel", "different-depth"):
        with TestClient(app) as client:

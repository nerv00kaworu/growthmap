...
def select_provider(client, provider):
 ...
 selected=response.json();assert selected['selection_revision']==provider['selection_revision']+1
...
def test_canonical_types_have_ts_parity_and_real_routes_persist(monkeypatch):
 ...
 with TestClient(app) as c:
  p=c.post('/api/projects',json={'name':'blocks'}).json();root=c.get('/api/nodes/'+p['root_node_id']).json();provider=select_provider(c,c.post('/api/providers',json={'name':'mock','provider_type':'mock'}).json())
  for typ in CONTENT_BLOCK_TYPES:
   ...
   deep=c.post('/api/ai/deepen',json={'node_id':root['id'],'provider_id':provider['id'],'provider_revision':provider['revision'],'selection_revision':provider['selection_revision']});assert deep.status_code==200,(typ,deep.text)
...
def test_invalid_deepen_is_502_and_builtin_mock_expand_deepen_create():
 with TestClient(app) as c:
  p=c.post('/api/projects',json={'name':'mock'}).json();root=c.get('/api/nodes/'+p['root_node_id']).json();provider=select_provider(c,c.post('/api/providers',json={'name':'mock','provider_type':'mock'}).json());q={'node_id':root['id'],'provider_id':provider['id'],'provider_revision':provider['revision'],'selection_revision':provider['selection_revision']}

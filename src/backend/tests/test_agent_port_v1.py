import importlib,os,sys,tempfile,unittest,uuid,json,subprocess
...
class AgentPortV1(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.tmp=tempfile.TemporaryDirectory();os.environ["DATABASE_URL"]=f"sqlite+aiosqlite:///{cls.tmp.name}/db.sqlite";os.environ["APP_ENV"]="test";sys.path.insert(0,str(Path(__file__).parents[1]));import main;cls.client=TestClient(main.app);cls.client.__enter__()
 @classmethod
 ...
 def grant(self,permission="write",scope=None,expired=False):
  body={"project_id":self.pid,"permission":permission,"expires_at":(datetime.now(timezone.utc)+(-timedelta(minutes=2) if expired else timedelta(hours=1))).isoformat(),"label":"test","agent_identity":"neutral-test"};body.update(scope or {});r=self.client.post("/api/agent-port/grants",json=body);return r

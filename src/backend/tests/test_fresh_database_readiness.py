...
def test_fresh_created_database_is_proven_before_readiness(tmp_path):
 ...
 stat=path.stat();identity={'device':stat.st_dev,'inode':stat.st_ino,'meaningful':bool(stat.st_dev or stat.st_ino)}
 proof=asyncio.run(verify_fresh_created(engine,SimpleNamespace(max_active_projects=1),identity))
...
def test_fresh_mutation_before_readiness_fails(tmp_path,kind):
 ...
 with pytest.raises(RuntimeError,match='identity mismatch|digest mismatch|exceeds active project entitlement'):asyncio.run(verify_fresh_created(engine,SimpleNamespace(max_active_projects=1),identity,inject))
 asyncio.run(engine.dispose())

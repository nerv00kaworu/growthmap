...
import pytest
ROOT=Path(__file__).parents[2];R15=ROOT.parents[0]/'reports/phase2-authority-production-readiness-20260808/vps-staging-r15';DEP=R15/'deploy'
def module(p,n):
...
def test_release_identity_strict_and_old_process_rejected(monkeypatch,tmp_path):
 ...
 a=module(DEP/'acceptance.py','r15accept');cfg=tmp_path/'composition.json';auth=tmp_path/'authority-production.json';auth.write_text(json.dumps({'reviewed_public_key_sha256':'1'*64}));cfg.write_text(json.dumps({'authority_config_path':str(auth),'listen':{'host':'127.0.0.1','port':8320}}))
...
def test_unit_exact_parity_and_stable_security_config():
 u=(DEP/'growthmap-authority-staging-r15.service.template').read_text();p=(R15/'provision_staging.py').read_text()
 assert 'python -I -B -m licensing.authority_composition --fixed-config /etc/growthmap-authority-staging-r15/production-composition.json --expected-service-uid @SERVICE_UID@ --release-digest @RELEASE_DIGEST@' in u
...
def test_registry_binds_authority_pin_and_venv(tmp_path):
 r=module(DEP/'registry.py','r15registry');release=tmp_path/('a'*64);release.mkdir();
 for n,v in [('runtime.tar',b'x'),('source-files.sha256',b'y'),('venv-manifest.json',b'z')]: (release/n).write_bytes(v)
 unit=tmp_path/'growthmap-authority-staging-r15.service';unit.write_text('u');auth=tmp_path/'authority-production.json';auth.write_text(json.dumps({'reviewed_public_key_sha256':'b'*64}));cfg=tmp_path/'production-composition.json';cfg.write_text(json.dumps({'expected_uid':123,'listen':{'port':8320},'edge':{'public_key_sha256':'c'*64}}))
...
def test_transaction_fresh_and_existing_byte_metadata_restore(tmp_path):
 t=module(DEP/'transaction.py','r15tx');root=tmp_path/'state';snap=tmp_path/'snap';t.snapshot(root,snap);root.mkdir();(root/'new').write_bytes(b'n');t.restore(root,snap);assert not root.exists()
...
def test_transaction_rejects_hardlink_and_external_symlink(tmp_path):
 t=module(DEP/'transaction.py','r15txbad');root=tmp_path/'state';root.mkdir();a=root/'a';a.write_text('x');os.link(a,root/'b')
...
def test_real_venv_copies_inventory_structural_policy(tmp_path):
 v=module(DEP/'venv_manifest.py','r15venvreal');root=tmp_path/'venv';subprocess.run([sys.executable,'-B','-m','venv','--copies',str(root)],check=True,env={**os.environ,'PYTHONDONTWRITEBYTECODE':'1'})
...
def test_venv_verifier_rejects_addition_symlink_hardlink_and_pyc(tmp_path):
 v=module(DEP/'venv_manifest.py','r15venv');root=tmp_path/'v';(root/'bin').mkdir(parents=True);(root/'bin/python3').write_bytes(b'elf');(root/'a').write_bytes(b'a');assert any(x['path']=='a' for x in v.inventory(root));(root/'x.pyc').write_bytes(b'x')
...
def _fake_apply_env(tmp_path,monkeypatch,fail=''):
 import shutil
 rail=tmp_path/'rail';shutil.copytree(R15,rail);root=tmp_path/'root';(root/'opt').mkdir(parents=True);(root/'var/lib').mkdir(parents=True);(root/'etc/systemd/system').mkdir(parents=True);bind=tmp_path/'bin';bind.mkdir();log=tmp_path/'calls';state=tmp_path/'systemctl-state';state.write_text('active enabled')
...
if name=='getent':print('growthmap-auth-stg:x:1234:1235::/nonexistent:/usr/sbin/nologin')
 ...
 if cmd=='stop':words.discard('active')
 ...
 if script and script.endswith('transaction.py'):os.execv(sys.executable,[sys.executable,script,*a[a.index(script)+1:]])
...
''');dispatcher.write_text(dispatcher.read_text().replace('#!/usr/bin/env python3','#!'+sys.executable));dispatcher.chmod(0o755)
 ...
 env={**os.environ,'PATH':str(bind)+':'+os.environ['PATH'],'R15_ROOT':str(root),'FAKE_LOG':str(log),'SYSTEMCTL_STATE':str(state),'FAIL_STAGE':fail,'FAKE_PYTHON3':str(bind/'python3'),'RAIL':str(rail)}
...
def test_entire_deploy_upgrade_and_rollback_active_lifecycle(tmp_path):
 ...
 current=root/'opt/growthmap-authority-staging-r15/current';assert os.readlink(current)=='releases/'+b
 assert subprocess.run(['/bin/sh',str(rail/'deploy/rollback.sh'),'--apply',a],env=env).returncode==0;assert os.readlink(current)=='releases/'+a and 'restart growthmap-authority-staging-r15.service' in log.read_text()

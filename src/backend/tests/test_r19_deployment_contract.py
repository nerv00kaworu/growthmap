...
import pytest
ROOT=Path(__file__).parents[2];R19=ROOT.parents[0]/'reports/phase2-authority-production-readiness-20260808/vps-staging-r19';DEP=R19/'deploy'
def module(p,n):
...
def test_release_identity_strict_and_old_process_rejected(monkeypatch,tmp_path):
 ...
 a=module(DEP/'acceptance.py','r19accept');cfg=tmp_path/'composition.json';auth=tmp_path/'authority-production.json';auth.write_text(json.dumps({'reviewed_public_key_sha256':'1'*64}));cfg.write_text(json.dumps({'authority_config_path':str(auth),'listen':{'host':'127.0.0.1','port':8320}}))
...
def test_unit_exact_parity_and_stable_security_config():
 u=(DEP/'growthmap-authority-staging-r19.service.template').read_text();p=(R19/'provision_staging.py').read_text()
 assert 'python -I -B -m licensing.authority_composition --fixed-config /etc/growthmap-authority-staging-r19/production-composition.json --expected-service-uid @SERVICE_UID@ --release-digest @RELEASE_DIGEST@' in u
...
def test_registry_binds_authority_pin_and_venv(tmp_path):
 r=module(DEP/'registry.py','r19registry');release=tmp_path/('a'*64);release.mkdir();
 for n,v in [('runtime.tar',b'x'),('source-files.sha256',b'y'),('venv-manifest.json',b'z')]: (release/n).write_bytes(v)
 unit=tmp_path/'growthmap-authority-staging-r19.service';unit.write_text('u');auth=tmp_path/'authority-production.json';auth.write_text(json.dumps({'reviewed_public_key_sha256':'b'*64}));cfg=tmp_path/'production-composition.json';cfg.write_text(json.dumps({'expected_uid':123,'listen':{'port':8320},'edge':{'public_key_sha256':'c'*64}}))
...
def test_transaction_fresh_and_existing_byte_metadata_restore(tmp_path):
 t=module(DEP/'transaction.py','r19tx');root=tmp_path/'state';snap=tmp_path/'snap';t.snapshot(root,snap);root.mkdir();(root/'new').write_bytes(b'n');t.restore(root,snap);assert not root.exists()
...
def test_transaction_rejects_hardlink_and_external_symlink(tmp_path):
 t=module(DEP/'transaction.py','r19txbad');root=tmp_path/'state';root.mkdir();a=root/'a';a.write_text('x');os.link(a,root/'b')
...
def test_real_venv_copies_inventory_structural_policy(tmp_path):
 v=module(DEP/'venv_manifest.py','r19venvreal');root=tmp_path/'venv';subprocess.run([sys.executable,'-B','-m','venv','--copies',str(root)],check=True,env={**os.environ,'PYTHONDONTWRITEBYTECODE':'1'})
...
def test_venv_verifier_rejects_addition_symlink_hardlink_and_pyc(tmp_path):
 v=module(DEP/'venv_manifest.py','r19venv');root=tmp_path/'v';(root/'bin').mkdir(parents=True);(root/'bin/python3').write_bytes(b'elf');(root/'a').write_bytes(b'a');assert any(x['path']=='a' for x in v.inventory(root));(root/'x.pyc').write_bytes(b'x')
...
def _fake_apply_env(tmp_path,monkeypatch,fail=''):
 import shutil
 rail=tmp_path/'rail';shutil.copytree(R19,rail);root=tmp_path/'root';(root/'opt').mkdir(parents=True);(root/'var/lib/growthmap-authority-staging-r19-transactions').mkdir(parents=True);(root/'etc/systemd/system').mkdir(parents=True);
 for script in (rail/'deploy/deploy.sh',rail/'deploy/rollback.sh'):
  text=script.read_text()
  bind=tmp_path/'bin'
  paths=[('/var/lib/growthmap-authority-staging-r19-transactions',str(root/'var/lib/growthmap-authority-staging-r19-transactions')),('/opt/growthmap-authority-staging-r19',str(root/'opt/growthmap-authority-staging-r19')),('/var/lib/growthmap-authority-staging-r19',str(root/'var/lib/growthmap-authority-staging-r19')),('/etc/growthmap-authority-staging-r19',str(root/'etc/growthmap-authority-staging-r19')),('/etc/systemd/system',str(root/'etc/systemd/system'))]
  for i,(canonical,adapted) in enumerate(paths):
   marker=f'__R19_ADAPTER_PATH_{i}__';assert marker not in text and canonical in text;text=text.replace(canonical,marker)
  for i,(_canonical,adapted) in enumerate(paths):text=text.replace(f'__R19_ADAPTER_PATH_{i}__',adapted)
  ...
  protected=('--tar','-m pip install','-Hln','source-files.sha256','R19_TRANSACTION_INJECT')
...
def test_entire_deploy_upgrade_and_rollback_active_lifecycle(tmp_path):
 ...
 current=root/'opt/growthmap-authority-staging-r19/current';assert os.readlink(current)=='releases/'+b
 trace=log.read_text()
 ...
 assert subprocess.run(['/bin/sh',str(rail/'deploy/rollback.sh'),'--apply',a],env=env).returncode==0;assert os.readlink(current)=='releases/'+a and 'restart growthmap-authority-staging-r19.service' in log.read_text()
...
def test_production_rejects_root_overrides():
 for script,args in [(DEP/'deploy.sh',[]),(DEP/'rollback.sh',['--apply','a'*64])]: 
  for key in ('R19_ROOT','R15_ROOT','GROWTHMAP_ROOT'):
...
def test_sqlite_fail_closed_wal_and_partial_cleanup(tmp_path,monkeypatch):
 t=module(DEP/'transaction.py','r19txsqlite');bad=tmp_path/'bad.db';bad.write_bytes(b'not sqlite');snap=tmp_path/'snap'
...
def test_transaction_rejects_overlap_and_symlink_parent(tmp_path):
 t=module(DEP/'transaction.py','r19txcontain');a=(tmp_path/'a').absolute();a.mkdir()
...
def test_relocated_console_script_and_no_incoming_bytes(tmp_path):
 r=module(DEP/'relocate_venv.py','r19relocate');incoming=tmp_path/'.incoming-x';release=tmp_path/'release';v=incoming/'venv';(v/'bin').mkdir(parents=True);(release/'venv/bin').mkdir(parents=True)
...
def test_private_temp_never_touches_predictable_symlink(tmp_path):
 ...
 sentinel=tmp_path/'sentinel';sentinel.write_text('safe');predictable=tmp_path/'growthmap-authority-staging-r19.service.1';predictable.symlink_to(sentinel)
 assert sentinel.read_text()=='safe'


def _r19_multi(tmp_path):
 t=module(DEP/'transaction.py','r19txmulti');roots=[]
 for i in range(4):
...
def test_r19_all_root_restore_and_prevalidation(tmp_path):
 t,roots,snap,jr=_r19_multi(tmp_path);t.restore_all(snap,roots,jr)
 assert [(p/'v').read_text() for p in roots]==['B0','B1','B2','B3'] and not list(jr.iterdir())

def test_r19_root_boundary_failures_undo_to_exact_candidate(tmp_path,monkeypatch):
 for point in [*(f'before_backup_{i}' for i in range(4)),*(f'after_backup_{i}' for i in range(4)),*(f'before_install_{i}' for i in range(4)),*(f'after_install_{i}' for i in range(4))]:
  case=tmp_path/point;case.mkdir();t,roots,snap,jr=_r19_multi(case);monkeypatch.setenv('R19_TRANSACTION_INJECT',point)
  ...
  monkeypatch.delenv('R19_TRANSACTION_INJECT')

def test_r19_undo_failure_leaves_journal_and_recover_is_idempotent(tmp_path,monkeypatch):
 t,roots,snap,jr=_r19_multi(tmp_path);original=t._fail
 def injected(point):
...
def test_r19_metadata_tamper_matrix_is_before_mutation(tmp_path):
 mutators=[
 ...
 for n,mutate in enumerate(mutators):
  case=tmp_path/str(n);case.mkdir();t,roots,snap,jr=_r19_multi(case);before=[(p/'v').read_text() for p in roots];mp=snap/'0/meta.json';m=json.loads(mp.read_text());mutate(m);mp.write_text(json.dumps(m))
  with pytest.raises(Exception,match='transaction'): t.restore_all(snap,roots,jr)
  assert [(p/'v').read_text() for p in roots]==before

def test_r19_complete_extra_hardlink_and_symlink_tamper(tmp_path):
 for kind in ('complete','extra','hardlink','symlink'):
  case=tmp_path/kind;case.mkdir();t,roots,snap,jr=_r19_multi(case)
...
def test_r19_privileged_environment_and_absolute_python_contract():
 ...
 u=(DEP/'growthmap-authority-staging-r19.service.template').read_text()
 assert 'Environment=PATH=/usr/sbin:/usr/bin:/sbin:/bin' in u and 'Environment=PYTHONPATH=' in u

def test_r19_hostile_environment_cannot_run_path_shim(tmp_path):
...
PRODUCTION_FILES=tuple(sorted([*DEP.glob('*'),R19/'provision_staging.py']))
PRODUCTION_FILES=tuple(p for p in PRODUCTION_FILES if p.is_file())
SERVICE=DEP/'growthmap-authority-staging-r19.service.template'
EXPECTED_SERVICE_LINES={
 'Description=GrowthMap Authority isolated staging R19',
 'ExecStart=/opt/growthmap-authority-staging-r19/current/venv/bin/python -I -B -m licensing.authority_composition --fixed-config /etc/growthmap-authority-staging-r19/production-composition.json --expected-service-uid @SERVICE_UID@ --release-digest @RELEASE_DIGEST@',
 'WorkingDirectory=/opt/growthmap-authority-staging-r19/current',
 'ReadOnlyPaths=/opt/growthmap-authority-staging-r19 /etc/growthmap-authority-staging-r19',
 'ReadWritePaths=/var/lib/growthmap-authority-staging-r19',
}
REVIEWED_EXECUTABLES={'python','python3','cp','readlink','dirname','pwd','cat','getent','cut','mktemp','stat','id','systemctl','rm','sha256sum','install','tar','find','chmod','mv','sed','systemd-analyze','ss','grep','ln','sh'}

def _audit_production(overrides=None):
 """Bounded byte/token audit, deliberately not a complete POSIX shell parser."""
 import re
 overrides=overrides or {};texts={p:overrides.get(p,p.read_text()) for p in PRODUCTION_FILES}
 for p,text in texts.items():
  for stale in ('staging-r16','staging-r17','staging-r18','R16','R17','R18'): assert stale not in text,(p,stale)
  for corrupt in ('--/','pip /','-H/'): assert corrupt not in text,(p,corrupt)
 # Legacy override is an exact, reviewed two-line allowlist; no other legacy identity is allowed.
 legacy=[(p,n,l) for p,t in texts.items() for n,l in enumerate(t.splitlines(),1) if 'R15_ROOT' in l]
 expected='[ -z "${R19_ROOT:-}" ]&&[ -z "${R15_ROOT:-}" ]&&[ -z "${GROWTHMAP_ROOT:-}" ]||{ echo root_override_rejected >&2;exit 2;}'
 assert legacy==[(DEP/'deploy.sh',9,expected),(DEP/'rollback.sh',9,expected)]
 unit=set(texts[SERVICE].splitlines());assert EXPECTED_SERVICE_LINES <= unit
 corpus='\n'.join(texts[p] for p in texts)
 for identity in ('/opt/growthmap-authority-staging-r19','/var/lib/growthmap-authority-staging-r19','/etc/growthmap-authority-staging-r19','/var/lib/growthmap-authority-staging-r19-transactions','growthmap-authority-staging-r19.service'):
  assert identity in corpus,identity
 # Review every absolute executable token in every shell helper and executable-bearing unit line.
 shell_files=[p for p in texts if p.suffix=='.sh']
 token_inventory=[]
 for p in [*shell_files,SERVICE]:
  for n,line in enumerate(texts[p].splitlines(),1):
   for m in re.finditer(r'/(?:usr/)?(?:s?bin)/([A-Za-z0-9_.-]+)',line):
    name=m.group(1);assert name in REVIEWED_EXECUTABLES,(p,n,name)
    embedded=(name=='python' and line[max(0,m.start()-5):m.start()]=='/venv')
    assert embedded or m.start()==0 or line[m.start()-1] in ' \t;|($!&=\")', (p,n,line)
    assert m.end()==len(line) or line[m.end()] in ' \t;|)&<>\"', (p,n,line)
    token_inventory.append((p.name,n,name))
 assert token_inventory and any(x[2]=='systemctl' for x in token_inventory) and any(x[2]=='python3' for x in token_inventory)
 # Exact reviewed high-risk argv/options and shell operators.
 deploy=texts[DEP/'deploy.sh'];rollback=texts[DEP/'rollback.sh']
 for exact in ('verify_bundle.py" --tar "$TMP/runtime.tar"','-m pip install --no-compile','/usr/bin/ss -Hln "sport = :$PORT"|/usr/bin/grep .','/usr/bin/tar -xf','/usr/bin/find ','/usr/bin/cut ','/usr/bin/cat ','/usr/bin/sed ','/usr/bin/mv ','/usr/bin/rm ','/usr/bin/systemctl '): assert exact in deploy+rollback,exact
 return token_inventory

def test_r19_all_publishable_production_bytes_identity_and_semantics():
 _audit_production()

def test_r19_adapter_global_substring_mutation_would_be_detected():
 text=(DEP/'deploy.sh').read_text()
 for old,new,signature in [('tar','/usr/bin/tar','--/usr/bin/tar'),('install','/usr/bin/install','pip /usr/bin/install'),('ln','/usr/bin/ln','-H/usr/bin/ln')]:
  corrupted=text.replace(old,new);assert signature in corrupted
  with pytest.raises(AssertionError):_audit_production({DEP/'deploy.sh':corrupted})

@pytest.mark.parametrize('old',('R18','R17'))
def test_r19_service_description_mutations_rejected(old):
 text=SERVICE.read_text().replace('isolated staging R19','isolated staging '+old)
 with pytest.raises(AssertionError):_audit_production({SERVICE:text})

@pytest.mark.parametrize('needle',(
 '/opt/growthmap-authority-staging-r19/current/venv/bin/python',
 '/opt/growthmap-authority-staging-r19/current',
 'ReadOnlyPaths=/opt/growthmap-authority-staging-r19 /etc/growthmap-authority-staging-r19',
 'ReadWritePaths=/var/lib/growthmap-authority-staging-r19',
 'growthmap-authority-staging-r19.service',
 '/var/lib/growthmap-authority-staging-r19-transactions',
 '/opt/growthmap-authority-staging-r19','/etc/growthmap-authority-staging-r19','/var/lib/growthmap-authority-staging-r19'))
def test_r19_identity_mutations_across_all_production_bytes_rejected(needle):
 overrides={p:p.read_text().replace(needle,needle.replace('r19','r18')) for p in PRODUCTION_FILES}
 assert any(overrides[p]!=p.read_text() for p in PRODUCTION_FILES)
 with pytest.raises(AssertionError):_audit_production(overrides)


def test_r19_real_plan_build_verify_wheels_and_recover(tmp_path):
 """Run canonical plan with real python/build/verify/checksums; adapt host identity and journal only."""
 import re,shutil
 work=tmp_path/'work';rail=work/'reports/phase2-authority-production-readiness-20260808/vps-staging-r19';rail.parent.mkdir(parents=True);shutil.copytree(R19,rail)
 ...
 text=text.replace('/var/lib/growthmap-authority-staging-r19-transactions',str(journal))

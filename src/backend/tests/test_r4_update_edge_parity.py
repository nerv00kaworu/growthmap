...
async def frozen(p,eid):
 async with async_session() as db:
  p1=await db.get(Project,p);e=await db.get(Edge,eid);edges=(await db.execute(select(Edge).where(Edge.project_id==p).order_by(Edge.id))).scalars().all()
  ...
  return {'project':(p1.id,p1.name,p1.description,p1.goal,p1.status,p1.root_node_id,p1.revision,dt(p1.created_at),dt(p1.updated_at)),'edge':(e.id,e.project_id,e.from_node_id,e.to_node_id,e.relation_type,e.weight,e.note,e.is_mainline,e.revision,dt(e.created_at)),'edges':[(x.id,x.project_id,x.from_node_id,x.to_node_id,x.relation_type,x.weight,x.note,x.is_mainline,x.revision,dt(x.created_at)) for x in edges],'logs':[(x.id,x.node_id,x.actor_type,x.actor_id,x.action_type,x.payload,dt(x.created_at)) for x in logs],'receipts':[(x.id,x.grant_id,x.idempotency_key,x.request_digest,x.action_type,x.status,x.response,dt(x.created_at)) for x in rs],'proposals':[(x.id,x.grant_id,x.target_node_id,x.status,x.title,x.rationale,x.expected_project_revision,x.operations,x.review_note,dt(x.created_at),dt(x.reviewed_at)) for x in qs]}
with TestClient(app,raise_server_exceptions=True) as c:
 ...
 p,a,b,e=mk(c,'home');h=grant(c,p);other,oa,ob,oe=mk(c,'foreign');before=arun(frozen(p,e['id']));foreign_before=arun(frozen(other,oe['id']));rev=c.get('/api/projects/'+p).json()['revision'];r=batch(c,h,p,'cross-exact',[op(oe,note='x')],rev);assert r.status_code==404 and r.json()['detail']=='Edge not found' and arun(frozen(p,e['id']))==before and arun(frozen(other,oe['id']))==foreign_before
 codes=[]
 for reverse in (False,True):
  p,a,b,e=mk(c,'dependency-exact'+str(reverse));h=grant(c,p);rev=c.get('/api/projects/'+p).json()['revision'];new=str(uuid.uuid4());create={'op':'create_edge','id':new,'from_node_id':a,'to_node_id':b,'expected_from_revision':c.get('/api/nodes/'+a).json()['revision'],'expected_to_revision':c.get('/api/nodes/'+b).json()['revision'],'relation_type':'references'};update={'op':'update_edge','edge_id':new,'expected_revision':1,'fields':{'note':'secret'}};before=arun(frozen(p,e['id']));assert new not in {x[0] for x in before['edges']};r=batch(c,h,p,'dependency-exact-'+str(reverse).lower(),[update,create] if reverse else [create,update],rev);codes.append(r.json()['detail']['code']);after=arun(frozen(p,e['id']));assert r.status_code==422 and after==before and new not in {x[0] for x in after['edges']}

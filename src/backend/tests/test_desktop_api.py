...
with TestClient(app) as client:
 ...
 assert client.patch(f"/api/projects/{p[0]['id']}",headers=h,json={'status':'archived','expected_project_revision':p[0]['revision']}).status_code==200
 ...
 assert client.patch(f"/api/projects/{p[0]['id']}",headers=h,json={'status':'active','expected_project_revision':p[0]['revision']+1}).status_code==409
'''

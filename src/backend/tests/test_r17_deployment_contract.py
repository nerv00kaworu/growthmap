...
import pytest
ROOT=Path(__file__).parents[2];R17=ROOT.parents[0]/'reports/phase2-authority-production-readiness-20260808/vps-staging-r17';DEP=R17/'deploy'
def module(p,n):
...
def test_release_identity_strict_and_old_process_rejected(monkeypatch,tmp_path):
 ...
 a=module(DEP/'acceptance.py','r17accept');cfg=tmp_path/'composition.json';auth=tmp_path/'authority-production.json';auth.write_text(json.dumps({'reviewed_public_key_sha256':'1'*64}));cfg.write_text(json.dumps({'authority_config_path':str(auth),'listen':{'host':'127.0.0.1','port':8320}}))
...
def test_unit_exact_parity_and_stable_security_config():
 u=(DEP/'growthmap-authority-staging-r17.service.template').read_text();p=(R17/'provision_staging.py').read_text()
 assert 'python -I -B -m licensing.authority_composition --fixed-config /etc/growthmap-authority-staging-r17/production-composition.json --expected-service-uid @SERVICE_UID@ --release-digest @RELEASE_DIGEST@' in u
...
def test_registry_binds_authority_pin_and_venv(tmp_path):
 r=module(DEP/'registry.py','r17registry');release=tmp_path/('a'*64);release.mkdir();
 for n,v in [('runtime.tar',b'x'),('source-files.sha256',b'y'),('venv-manifest.json',b'z')]: (release/n).write_bytes(v)
 unit=tmp_path/'growthmap-authority-staging-r17.service';unit.write_text('u');auth=tmp_path/'authority-production.json';auth.write_text(json.dumps({'reviewed_public_key_sha256':'b'*64}));cfg=tmp_path/'production-composition.json';cfg.write_text(json.dumps({'expected_uid':123,'listen':{'port':8320},'edge':{'public_key_sha256':'c'*64}}))
...
def test_transaction_fresh_and_existing_byte_metadata_restore(tmp_path):
 t=module(DEP/'transaction.py','r17tx');root=tmp_path/'state';snap=tmp_path/'snap';t.snapshot(root,snap);root.mkdir();(root/'new').write_bytes(b'n');t.restore(root,snap);assert not root.exists()
...
def test_transaction_rejects_hardlink_and_external_symlink(tmp_path):
 t=module(DEP/'transaction.py','r17txbad');root=tmp_path/'state';root.mkdir();a=root/'a';a.write_text('x');os.link(a,root/'b')
...
def test_real_venv_copies_inventory_structural_policy(tmp_path):
 v=module(DEP/'venv_manifest.py','r17venvreal');root=tmp_path/'venv';subprocess.run([sys.executable,'-B','-m','venv','--copies',str(root)],check=True,env={**os.environ,'PYTHONDONTWRITEBYTECODE':'1'})
...
def test_venv_verifier_rejects_addition_symlink_hardlink_and_pyc(tmp_path):
 v=module(DEP/'venv_manifest.py','r17venv');root=tmp_path/'v';(root/'bin').mkdir(parents=True);(root/'bin/python3').write_bytes(b'elf');(root/'a').write_bytes(b'a');assert any(x['path']=='a' for x in v.inventory(root));(root/'x.pyc').write_bytes(b'x')
...
def _fake_apply_env(tmp_path,monkeypatch,fail=''):
 import shutil
 rail=tmp_path/'rail';shutil.copytree(R17,rail);root=tmp_path/'root';(root/'opt').mkdir(parents=True);(root/'var/lib').mkdir(parents=True);(root/'etc/systemd/system').mkdir(parents=True);
 for script in (rail/'deploy/deploy.sh',rail/'deploy/rollback.sh'):
  text=script.read_text()
  bind=tmp_path/'bin'
  for canonical,adapted in [('/var/lib/growthmap-authority-staging-r17-transactions',str(root/'var/lib/growthmap-authority-staging-r17-transactions')),('/opt/growthmap-authority-staging-r17',str(root/'opt/growthmap-authority-staging-r17')),('/var/lib/growthmap-authority-staging-r17',str(root/'var/lib/growthmap-authority-staging-r17')),('/etc/growthmap-authority-staging-r17',str(root/'etc/growthmap-authority-staging-r17')),('/etc/systemd/system',str(root/'etc/systemd/system'))]:text=text.replace(canonical,adapted)
  absolute=['python3','getent','systemctl','systemd-analyze','ss','tar','install','chmod']
  for tool in absolute:
   token='/usr/bin/'+tool
   assert token in text or tool in ('systemd-analyze','ss','tar','chmod')
   text=text.replace(token,str(bind/tool))
...
def test_entire_deploy_upgrade_and_rollback_active_lifecycle(tmp_path):
 ...
 current=root/'opt/growthmap-authority-staging-r17/current';assert os.readlink(current)=='releases/'+b
 assert subprocess.run(['/bin/sh',str(rail/'deploy/rollback.sh'),'--apply',a],env=env).returncode==0;assert os.readlink(current)=='releases/'+a and 'restart growthmap-authority-staging-r17.service' in log.read_text()
...
def test_production_rejects_root_overrides():
 for script,args in [(DEP/'deploy.sh',[]),(DEP/'rollback.sh',['--apply','a'*64])]: 
  for key in ('R17_ROOT','R15_ROOT','GROWTHMAP_ROOT'):
...
def test_sqlite_fail_closed_wal_and_partial_cleanup(tmp_path,monkeypatch):
 t=module(DEP/'transaction.py','r17txsqlite');bad=tmp_path/'bad.db';bad.write_bytes(b'not sqlite');snap=tmp_path/'snap'
...
def test_transaction_rejects_overlap_and_symlink_parent(tmp_path):
 t=module(DEP/'transaction.py','r17txcontain');a=(tmp_path/'a').absolute();a.mkdir()
...
def test_relocated_console_script_and_no_incoming_bytes(tmp_path):
 r=module(DEP/'relocate_venv.py','r17relocate');incoming=tmp_path/'.incoming-x';release=tmp_path/'release';v=incoming/'venv';(v/'bin').mkdir(parents=True);(release/'venv/bin').mkdir(parents=True)
...
def test_private_temp_never_touches_predictable_symlink(tmp_path):
 ...
 sentinel=tmp_path/'sentinel';sentinel.write_text('safe');predictable=tmp_path/'growthmap-authority-staging-r17.service.1';predictable.symlink_to(sentinel)
 assert sentinel.read_text()=='safe'


def _r17_multi(tmp_path):
 t=module(DEP/'transaction.py','r17txmulti');roots=[]
 for i in range(4):
...
def test_r17_all_root_restore_and_prevalidation(tmp_path):
 t,roots,snap,jr=_r17_multi(tmp_path);t.restore_all(snap,roots,jr)
 assert [(p/'v').read_text() for p in roots]==['B0','B1','B2','B3'] and not list(jr.iterdir())

def test_r17_root_boundary_failures_undo_to_exact_candidate(tmp_path,monkeypatch):
 for point in [*(f'before_backup_{i}' for i in range(4)),*(f'after_backup_{i}' for i in range(4)),*(f'before_install_{i}' for i in range(4)),*(f'after_install_{i}' for i in range(4))]:
  case=tmp_path/point;case.mkdir();t,roots,snap,jr=_r17_multi(case);monkeypatch.setenv('R17_TRANSACTION_INJECT',point)
  ...
  monkeypatch.delenv('R17_TRANSACTION_INJECT')

def test_r17_undo_failure_leaves_journal_and_recover_is_idempotent(tmp_path,monkeypatch):
 t,roots,snap,jr=_r17_multi(tmp_path);original=t._fail
 def injected(point):
...
def test_r17_metadata_tamper_matrix_is_before_mutation(tmp_path):
 mutators=[
 ...
 for n,mutate in enumerate(mutators):
  case=tmp_path/str(n);case.mkdir();t,roots,snap,jr=_r17_multi(case);before=[(p/'v').read_text() for p in roots];mp=snap/'0/meta.json';m=json.loads(mp.read_text());mutate(m);mp.write_text(json.dumps(m))
  with pytest.raises(Exception,match='transaction'): t.restore_all(snap,roots,jr)
  assert [(p/'v').read_text() for p in roots]==before

def test_r17_complete_extra_hardlink_and_symlink_tamper(tmp_path):
 for kind in ('complete','extra','hardlink','symlink'):
  case=tmp_path/kind;case.mkdir();t,roots,snap,jr=_r17_multi(case)
...
def test_r17_privileged_environment_and_absolute_python_contract():
 ...
 u=(DEP/'growthmap-authority-staging-r17.service.template').read_text()
 assert 'Environment=PATH=/usr/sbin:/usr/bin:/sbin:/bin' in u and 'Environment=PYTHONPATH=' in u

def test_r17_hostile_environment_cannot_run_path_shim(tmp_path):

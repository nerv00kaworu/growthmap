...
async def validated_scoped_graph(db,project_id,root_id,limit=5000,allow_external_root_parent=False):
    ...
    Node-scope projection is intentionally a singleton, so its ordinary parent
    edge may sit outside the projected scope. Descendants still require exact
    single-parent ownership. Branch scopes do not permit an external root edge.
    """
    ...
    # Outgoing traversal alone cannot see a second parent outside the closure.
    # Validate every incoming edge by identity, including endpoint truth.
    ...
    source_nodes={n.id:n for n in (await db.execute(select(Node).where(

...
test("branch selection clears pending update delete reparent undo and undo stays inert", async () => {
  ...
  const branch={id:"other",project_id:"p",source_node_id:"root",name:"other",status:"active",created_at:"",updated_at:"",revision:1} as never;
...
test("AI producers are owner and request generation fenced across switches and overlap",async()=>{
 ...
 try{for(const kind of ["expand","deepen"] as const)for(const disposition of ["success","reject"] as const){reset({aiLoading:false,expandSuggestions:null,expandTargetNodeId:null,deepenResult:null,error:null});const old=deferred<never>();if(kind==="expand")api.expand=(()=>old.promise) as typeof api.expand;else api.deepen=(()=>old.promise) as typeof api.deepen;const operation=kind==="expand"?useStore.getState().expandNode("root"):useStore.getState().deepenNode("root");await switchAtoBtoA();if(disposition==="success")old.resolve(kind==="expand"?{suggestions:[{title:"old",summary:"",node_type:"idea"}]} as never:{enriched_summary:"old",content_blocks:[]} as never);else old.reject(new Error("old error"));await operation;const state=useStore.getState();assert.equal(state.aiLoading,false);assert.equal(state.expandSuggestions,null);assert.equal(state.expandTargetNodeId,null);assert.equal(state.deepenResult,null);assert.equal(state.error,null)}reset();const first=deferred<never>(),second=deferred<never>();let call=0;api.expand=(()=>++call===1?first.promise:second.promise) as typeof api.expand;const a=useStore.getState().expandNode("root"),b=useStore.getState().expandNode("root");first.resolve({suggestions:[{title:"old",summary:"",node_type:"idea"}]} as never);await a;assert.equal(useStore.getState().aiLoading,true);second.resolve({suggestions:[{title:"new",summary:"",node_type:"idea"}]} as never);await b;assert.equal(useStore.getState().expandSuggestions?.[0].title,"new");assert.equal(useStore.getState().aiLoading,false);const branch={id:"x",project_id:"p",source_node_id:"root",name:"x",revision:1} as never;reset({branches:[branch]});const pending=deferred<never>();api.deepen=(()=>pending.promise) as typeof api.deepen;const oldBranch=useStore.getState().deepenNode("root");await useStore.getState().selectBranch(branch);pending.resolve({enriched_summary:"old",content_blocks:[]} as never);await oldBranch;assert.equal(useStore.getState().deepenResult,null)}finally{api.expand=oe;api.deepen=od;api.getSubtree=os;api.listBranches=ol;api.getBranchSubtree=obt;}
});
...
test("real cache losing branch subtree high node revision cannot poison next block",async()=>{const of=globalThis.fetch,restoreApi=useFreshApiClient();api.rememberResponse(project);api.rememberResponse(root);const one={id:"one",project_id:"p",source_node_id:"root",name:"one",revision:4},two={...one,id:"two",revision:2};const old=deferred<Response>();let posted:Record<string,unknown>={};globalThis.fetch=(async(input,init)=>{const url=String(input);if(url.endsWith("/branches/one/subtree"))return old.promise;if(url.endsWith("/branches/two/subtree"))return response({branch:two,tree:{...root,revision:5}});if(url.endsWith("/nodes/root/blocks")){posted=JSON.parse(String(init?.body));return response({id:"block",node_id:"root",block_type:"paragraph",content:{},order_index:0,revision:1})}throw Error(url)}) as typeof fetch;try{reset({branches:[one,two],currentBranch:null});const losing=useStore.getState().selectBranch(one as never);await Promise.resolve();await useStore.getState().selectBranch(two as never);old.resolve(response({branch:{...one,revision:99},tree:{...root,revision:99}}));await losing;await api.createBlock("root",{block_type:"paragraph",content:{body:"x"}});assert.deepEqual([posted.expected_project_revision,posted.expected_node_revision],[7,5])}finally{globalThis.fetch=of;restoreApi();}});

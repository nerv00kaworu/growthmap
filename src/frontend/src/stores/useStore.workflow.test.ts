...
test("AI producers are owner and request generation fenced across switches and overlap",async()=>{
 ...
 try{for(const kind of ["expand","deepen"] as const)for(const disposition of ["success","reject"] as const){reset({aiLoading:false,expandSuggestions:null,expandTargetNodeId:null,deepenResult:null,error:null});const old=deferred<never>();if(kind==="expand")api.expand=(()=>old.promise) as typeof api.expand;else api.deepen=(()=>old.promise) as typeof api.deepen;const operation=kind==="expand"?useStore.getState().expandNode("root",{providerId:"mock",providerType:"mock",model:"demo",revision:1}):useStore.getState().deepenNode("root",{providerId:"mock",providerType:"mock",model:"demo",revision:1});await switchAtoBtoA();if(disposition==="success")old.resolve(kind==="expand"?{suggestions:[{title:"old",summary:"",node_type:"idea"}]} as never:{enriched_summary:"old",content_blocks:[]} as never);else old.reject(new Error("old error"));await operation;const state=useStore.getState();assert.equal(state.aiLoading,false);assert.equal(state.expandSuggestions,null);assert.equal(state.expandTargetNodeId,null);assert.equal(state.deepenResult,null);assert.equal(state.error,null)}reset();const first=deferred<never>(),second=deferred<never>();let call=0;api.expand=(()=>++call===1?first.promise:second.promise) as typeof api.expand;const a=useStore.getState().expandNode("root",{providerId:"mock",providerType:"mock",model:"demo",revision:1}),b=useStore.getState().expandNode("root",{providerId:"mock",providerType:"mock",model:"demo",revision:1});first.resolve({suggestions:[{title:"old",summary:"",node_type:"idea"}]} as never);await a;assert.equal(useStore.getState().aiLoading,true);second.resolve({suggestions:[{title:"new",summary:"",node_type:"idea"}]} as never);await b;assert.equal(useStore.getState().expandSuggestions?.[0].title,"new");assert.equal(useStore.getState().aiLoading,false);const branch={id:"x",project_id:"p",source_node_id:"root",name:"x",revision:1} as never;reset({branches:[branch]});const pending=deferred<never>();api.deepen=(()=>pending.promise) as typeof api.deepen;const oldBranch=useStore.getState().deepenNode("root",{providerId:"mock",providerType:"mock",model:"demo",revision:1});await useStore.getState().selectBranch(branch);pending.resolve({enriched_summary:"old",content_blocks:[]} as never);await oldBranch;assert.equal(useStore.getState().deepenResult,null)}finally{api.expand=oe;api.deepen=od;api.getSubtree=os;api.listBranches=ol;api.getBranchSubtree=obt;}
});

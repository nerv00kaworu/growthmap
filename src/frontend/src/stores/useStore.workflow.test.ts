...
test("accept all final refresh rejection reports committed save without duplicate encouragement", async()=>{
  ...
  globalThis.confirm=()=>true; api.createNode=(async(_p,b)=>{creates+=1;return {...root,id:"made",title:b.title,authoritative_project_revision:8,authoritative_parent_id:"root",authoritative_parent_revision:4} as never}) as typeof api.createNode;
...
test("durable child undo uses exact authority before refresh and retires", async () => {
 ...
 const child={...root,id:"child",title:"child",revision:5,authoritative_project_revision:8,authoritative_parent_id:"root",authoritative_parent_revision:4} as GNode;
 api.createNode=(async()=>{events.push("create");return child}) as typeof api.createNode;api.deleteNode=(async(...x)=>{events.push("delete");args=x}) as typeof api.deleteNode;api.getProject=(async()=>{events.push("refresh");return {...project,revision:9}}) as typeof api.getProject;api.getSubtree=(async()=>root) as typeof api.getSubtree;
 try{reset({undoStack:[],currentBranch:null});await useStore.getState().addChildNode("root","child");assert.deepEqual(useStore.getState().undoStack[0].inverse,{kind:"delete-created-node",nodeId:"child",nodeRevision:5,projectRevision:8});await useStore.getState().undo();assert.deepEqual(args,["child",8,5]);assert.ok(events.indexOf("delete")<events.indexOf("refresh"));assert.deepEqual(useStore.getState().undoStack,[]);assert.equal(useStore.getState().rootNode?.children.length,0)}finally{api.createNode=oc;api.deleteNode=od;api.getProject=op;api.getSubtree=os;}
});

test("unsafe child create authority never publishes inverse",async()=>{const oc=api.createNode;try{for(const bad of [{...root,id:"x",revision:2,authoritative_parent_id:"root"},{...root,id:"x",revision:2,authoritative_project_revision:8,authoritative_parent_id:"wrong"},{...root,id:"x",project_id:"wrong",revision:2,authoritative_project_revision:8,authoritative_parent_id:"root"}]){reset({undoStack:[],currentBranch:null});api.createNode=(async()=>bad as never) as typeof api.createNode;await useStore.getState().addChildNode("root","x");assert.deepEqual(useStore.getState().undoStack,[])}}finally{api.createNode=oc;}});

test("undo failure retains inverse; committed delete with refresh failure retires and removes local child",async()=>{const oc=api.createNode,od=api.deleteNode,op=api.getProject,os=api.getSubtree;const made={...root,id:"made",revision:2,authoritative_project_revision:8,authoritative_parent_id:"root",authoritative_parent_revision:4} as GNode;try{api.createNode=(async()=>made) as typeof api.createNode;reset({undoStack:[],currentBranch:null});await useStore.getState().addChildNode("root","made");api.deleteNode=(async()=>{throw Error("delete failed")}) as typeof api.deleteNode;api.getProject=(async()=>({...project,revision:8})) as typeof api.getProject;api.getSubtree=(async()=>({...root,children:[made]})) as typeof api.getSubtree;await useStore.getState().undo();assert.equal(useStore.getState().undoStack.length,1);assert.equal(useStore.getState().rootNode?.children[0]?.id,"made");let deletes=0;api.deleteNode=(async()=>{deletes++}) as typeof api.deleteNode;api.getProject=(async()=>{throw Error("readback")}) as typeof api.getProject;await useStore.getState().undo();assert.equal(deletes,1);assert.deepEqual(useStore.getState().undoStack,[]);assert.equal(useStore.getState().rootNode?.children.length,0);await useStore.getState().undo();assert.equal(deletes,1);assert.match(useStore.getState().toast??"",/reload|Reload|重新載入|重新加载/)}finally{api.createNode=oc;api.deleteNode=od;api.getProject=op;api.getSubtree=os;}});
...
test("concurrent undo reserves one entry and never dispatches the next", async () => {
 ...
 api.getProject=(async()=>({...project,revision:10})) as typeof api.getProject;api.getSubtree=(async()=>root) as typeof api.getSubtree;
 try{reset({undoStack:[],currentBranch:null});for(const [id,rev] of [["one",8],["two",9]] as const){api.createNode=(async()=>({...root,id,revision:2,authoritative_project_revision:rev,authoritative_parent_id:"root",authoritative_parent_revision:4})) as typeof api.createNode;await useStore.getState().addChildNode("root",id)}api.deleteNode=(()=>{deletes++;return gate.promise}) as typeof api.deleteNode;const a=useStore.getState().undo(),b=useStore.getState().undo();assert.equal(deletes,1);assert.equal(useStore.getState().undoStack.length,1);gate.resolve();await Promise.all([a,b]);assert.equal(deletes,1)}finally{api.createNode=oc;api.deleteNode=od;api.getProject=op;api.getSubtree=os;}
});

test("failed undo restores in order around concurrent push; superseded and throw do not restore",async()=>{
 const oc=api.createNode,od=api.deleteNode,op=api.getProject,os=api.getSubtree;try{reset({undoStack:[],currentBranch:null});api.createNode=(async()=>({...root,id:"old",revision:2,authoritative_project_revision:8,authoritative_parent_id:"root",authoritative_parent_revision:4})) as typeof api.createNode;await useStore.getState().addChildNode("root","old");const gate=deferred<void>();api.deleteNode=(()=>gate.promise.then(()=>{throw Error("no")})) as typeof api.deleteNode;api.getProject=(async()=>({...project,revision:8})) as typeof api.getProject;api.getSubtree=(async()=>root) as typeof api.getSubtree;const old=useStore.getState().undo();api.createNode=(async()=>({...root,id:"new",revision:2,authoritative_project_revision:9,authoritative_parent_id:"root",authoritative_parent_revision:5})) as typeof api.createNode;await useStore.getState().addChildNode("root","new");gate.resolve();await old;assert.deepEqual(useStore.getState().undoStack.map(x=>x.inverse?.nodeId),["new","old"]);for(const mode of ["superseded","throw"] as const){reset({undoStack:[],currentBranch:null});api.createNode=(async()=>({...root,id:mode,revision:2,authoritative_project_revision:8,authoritative_parent_id:"root",authoritative_parent_revision:4})) as typeof api.createNode;await useStore.getState().addChildNode("root",mode);api.deleteNode=(async()=>{throw Error("no")}) as typeof api.deleteNode;const refresh=useStore.getState().refreshTree;useStore.setState({refreshTree:(async()=>{if(mode==="throw")throw Error("read");return "superseded"}) as never});await useStore.getState().undo();assert.deepEqual(useStore.getState().undoStack,[]);useStore.setState({refreshTree:refresh})}}finally{api.createNode=oc;api.deleteNode=od;api.getProject=op;api.getSubtree=os;}
});

test("create authority rejects collisions and hostile numerics; accepts main and exact branch",async()=>{const oc=api.createNode;const existing={...root,id:"existing"} as GNode;try{for(const bad of [undefined,NaN,Infinity,-Infinity,1.5,0,-1,Number.MAX_SAFE_INTEGER+1]){reset({undoStack:[],currentBranch:null});api.createNode=(async()=>({...root,id:"made",revision:bad,authoritative_project_revision:8,authoritative_parent_id:"root",authoritative_parent_revision:4})) as typeof api.createNode;await useStore.getState().addChildNode("root","x");assert.equal(useStore.getState().undoStack.length,0)}for(const id of ["root","existing"]){reset({rootNode:{...root,children:[existing]},undoStack:[],currentBranch:null});api.createNode=(async()=>({...root,id,revision:2,authoritative_project_revision:8,authoritative_parent_id:"root",authoritative_parent_revision:4})) as typeof api.createNode;await useStore.getState().addChildNode("root","x");assert.equal(useStore.getState().undoStack.length,0)}for(const branch of [null,{id:"br"}] as const){reset({undoStack:[],currentBranch:branch as never});api.createNode=(async()=>({...root,id:`ok-${branch?.id??"main"}`,branch_id:branch?.id,revision:2,authoritative_project_revision:8,authoritative_parent_id:"root",authoritative_parent_revision:4})) as typeof api.createNode;await useStore.getState().addChildNode("root","ok");assert.equal(useStore.getState().undoStack.length,1)}}finally{api.createNode=oc;}});

test("create settlement collision fails closed",async()=>{const oc=api.createNode,pending=deferred<never>();try{reset({undoStack:[],currentBranch:null});api.createNode=(()=>pending.promise) as typeof api.createNode;const create=useStore.getState().addChildNode("root","x");useStore.setState({rootNode:{...root,children:[{...root,id:"claimed"} as GNode]}});pending.resolve({...root,id:"claimed",revision:2,authoritative_project_revision:8,authoritative_parent_id:"root",authoritative_parent_revision:4} as never);await create;assert.deepEqual(useStore.getState().undoStack,[])}finally{api.createNode=oc;}});
...
test("undo delayed success after A-B-A has exact one delete and no stale publication",async()=>{const oc=api.createNode,od=api.deleteNode,os=api.getSubtree,ol=api.listBranches,gate=deferred<void>();let deletes=0;api.getSubtree=(async(id)=>({...root,id,project_id:id==="b-root"?"b":"p",title:"fresh"})) as typeof api.getSubtree;api.listBranches=(async()=>[]) as typeof api.listBranches;try{reset({undoStack:[],currentBranch:null,toast:null});api.createNode=(async()=>({...root,id:"old",revision:2,authoritative_project_revision:8,authoritative_parent_id:"root",authoritative_parent_revision:4})) as typeof api.createNode;await useStore.getState().addChildNode("root","old");api.deleteNode=(()=>{deletes++;return gate.promise}) as typeof api.deleteNode;const old=useStore.getState().undo();await switchAtoBtoA();gate.resolve();await old;assert.equal(deletes,1);assert.equal(useStore.getState().rootNode?.title,"fresh");assert.equal(useStore.getState().toast,null);assert.deepEqual(useStore.getState().undoStack,[])}finally{api.createNode=oc;api.deleteNode=od;api.getSubtree=os;api.listBranches=ol;}});

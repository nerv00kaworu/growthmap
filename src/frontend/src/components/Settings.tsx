...
export function Settings({ onClose }: SettingsProps) {
  ...
  const saveProfile = async () => {
    ...
    try{if(apiKey.trim()&&saved.provider_type!=="mock"){if(window.growthmapDesktop)await window.growthmapDesktop.secrets.set(saved.id,apiKey.trim());else await api.writeProviderSecret(saved.id,apiKey.trim(),saved.revision,Array.from(crypto.getRandomValues(new Uint8Array(24)),b=>b.toString(16).padStart(2,'0')).join(''));setApiKey("")}setSaveState(state=>reduceSettingsSave(state,{type:"SECRET_OK"}))}
  ...
  const recoverCredential=async(operation:"set"|"delete")=>{
    ...
    try{await (window.growthmapDesktop?window.growthmapDesktop.secrets.recover(selectedProfile.id,selectedProfile.revision,operation,operation==="set"?apiKey.trim():undefined,selectedProfile.secret_change_operation_id??undefined):api.recoverProviderSecret(selectedProfile.id,selectedProfile.revision,operation,selectedProfile.secret_change_operation_id,operation==="set"?apiKey.trim():undefined));setApiKey("");await loadProfiles();setMessage(u("✅ 憑證更新已完成。","✅ 凭据更新已完成。","✅ Credential update completed."))}catch{setMessage(u("憑證更新仍未完成；請重新輸入 key 或重試移除。","凭据更新仍未完成；请重新输入 key 或重试移除。","Credential update is still incomplete; re-enter the key or retry removal."))}finally{setSaving(false)}
  };

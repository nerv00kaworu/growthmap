      ...
      <Section title="AI 生長" subtitle="切換模式，避免所有衍生結果都走同一種形狀。" tone="ai">
        <div className="surface-subtle rounded-lg p-3 space-y-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <select
              ...
              className="rounded px-2 py-1 text-[11px] text-[var(--text-primary)] surface-subtle"
            >
          ...
          <p className="text-[11px] text-[var(--text-faint)]">{GROWTH_MODE_HELP[aiMode]}</p>
        </div>
        <input
          value={aiInstruction}
          onChange={(e) => setAiInstruction(e.target.value)}
          placeholder="可選：給 AI 的指示..."
          className="w-full surface-subtle rounded px-3 py-2 text-xs text-[var(--text-primary)] placeholder:text-[var(--text-faint)] focus:border-blue-500 focus:outline-none"
        />
        <div className="flex gap-2">
          <button
            ...
            disabled={aiLoading}
            className="flex-1 px-3 py-2 bg-purple-900/60 hover:bg-purple-800 disabled:bg-gray-800 disabled:text-gray-600 text-purple-200 text-xs rounded-lg transition-colors border border-purple-700/50"
          >
            {aiLoading ? "⏳ 生長中..." : "🌱 展開分支"}
          </button>
          <button
            ...
            disabled={aiLoading}
            className="flex-1 px-3 py-2 bg-teal-900/60 hover:bg-teal-800 disabled:bg-gray-800 disabled:text-gray-600 text-teal-200 text-xs rounded-lg transition-colors border border-teal-700/50"
          >
            {aiLoading ? "⏳ 深化中..." : "🔍 深化內容"}
          </button>
        </div>
      ...
      {expandSuggestions && expandSuggestions.length > 0 && (
        <Section title="分支建議" subtitle="先挑最有價值的分支採用，不要一次全收進來。" tone="ai">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={dismissAI} className="text-xs text-[var(--text-faint)] hover:text-[var(--text-primary)]">✕ 關閉</button>
          </div>
        ...
        <Section title="深化建議" subtitle="AI 先補內文骨架，再由你決定是否正式寫入。" tone="ai">
          ...
          <div className="bg-gray-800/80 border border-teal-800/40 rounded-lg p-3 space-y-2">
            <div>
              ...
              <p className="text-sm text-[var(--text-primary)] mt-1">{deepenResult.enriched_summary}</p>
            </div>

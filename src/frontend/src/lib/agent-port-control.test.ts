...
test("direct panel fails closed before API calls or token creation",()=>{assert.ok(panel.indexOf('data-testid="agent-port-unavailable"')<panel.indexOf('data-testid="agent-port-panel"'));assert.match(panel,/const reload=.*if\(!desktopControl\)return;.*agentPort\.list.*agentPort\.activity/);assert.match(panel,/const create=.*if\(!desktopControl\)return;.*agentPort\.create/);assert.match(panel,/const review=.*if\(!desktopControl\)return;.*agentPort\.review/);});

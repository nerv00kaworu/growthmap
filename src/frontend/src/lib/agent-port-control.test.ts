...
test("readback panel binds typed view model, evidence semantics, and node selection",()=>{assert.match(panel,/readbackViewModel/);assert.match(panel,/view\.revisionLabel/);assert.match(panel,/view\.stateLabel/);assert.match(panel,/view\.digestLabel/);assert.match(panel,/aria-label="Implementation evidence"/);assert.match(panel,/onSelectNode\?:\(nodeId:string\)=>void/);assert.match(page,/onSelectNode=\{selectNode\}/);});

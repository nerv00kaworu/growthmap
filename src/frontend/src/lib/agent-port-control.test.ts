...
test("readback panel exposes revision provenance, stale/current state, and complete evidence",()=>{assert.match(panel,/based on project r/);assert.match(panel,/context_stale/);assert.match(panel,/current r/);assert.match(panel,/context_snapshot_digest\.slice\(0,12\)/);for(const field of ["commits","files","tests","decisions","risks","todos","evidence"])assert.match(panel,new RegExp(`list\\("${field}"`));assert.match(panel,/onSelectNode\?:\(nodeId:string\)=>void/);});

...
export interface AIProviderIdentity { providerId:string; providerType:LLMProviderType; model:string; revision:number }
export function providerIdentity(p:ProviderConfig):AIProviderIdentity{return{providerId:p.id,providerType:p.provider_type as LLMProviderType,model:p.model_name,revision:p.revision}}
export function sameProviderIdentity(a:AIProviderIdentity|null,b:AIProviderIdentity|null){return!!(a&&b&&a.providerId===b.providerId&&a.providerType===b.providerType&&a.model===b.model&&a.revision===b.revision)}
...
export type AIPanelState={profiles:ProviderConfig[];profileState:"loading"|"ready"|"error";saving:boolean;modelError:string|null;test:PanelTestState|null;copyMessage:string|null};
export type AIPanelDeps={
 listProviders:()=>Promise<ProviderConfig[]>; updateModel:(id:string,model:string)=>Promise<ProviderConfig>; testConnection:(id:string,revision:number)=>Promise<{code?:string;request_id?:string}>;
 ...
 onSaved:(p:ProviderConfig)=>void; onInvalidate:()=>void;
};
function panelMessage(locale:AILocale,key:"save_error"|"test_confirm"|"test_success"|"copy_success"|"copy_error"){const m=(tw:string,cn:string,en:string)=>msg(locale,{"zh-TW":tw,"zh-CN":cn,en});switch(key){case"save_error":return m("模型儲存失敗，舊模型仍在使用。","模型保存失败，旧模型仍在使用。","Model save failed; the previous model remains active.");case"test_confirm":return m("測試會送出小型 completion，可能消耗額度。繼續？","测试会发送小型 completion，可能消耗额度。继续？","The test sends a small completion and may consume quota. Continue?");case"test_success":return m("連線成功。","连接成功。","Connection succeeded.");case"copy_success":return m("已複製請求 ID。","已复制请求 ID。","Request ID copied.");case"copy_error":return m("無法複製請求 ID。","无法复制请求 ID。","Could not copy request ID.")}}
...
export function createAIPanelController(d:AIPanelDeps){
 let alive=true,listGen=0,saveGen=0,testGen=0,state:AIPanelState={profiles:[],profileState:"loading",saving:false,modelError:null,test:null,copyMessage:null};const listeners=new Set<()=>void>();
 ...
 return {getSnapshot:()=>state,subscribe:(f:()=>void)=>{listeners.add(f);return()=>listeners.delete(f)},
  async list(){const g=++listGen;emit({profileState:"loading",test:null});try{const rows=await d.listProviders();if(alive&&g===listGen)emit({profiles:rows.filter(p=>p.enabled),profileState:"ready"})}catch{if(alive&&g===listGen)emit({profiles:[],profileState:"error"})}},
  ...
  invalidate(){listGen++;saveGen++;testGen++;emit({saving:false,test:null,modelError:null})},
  activate(){alive=true},

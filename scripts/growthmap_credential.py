...
def _api():
 ...
 k.SecureZeroMemory.argtypes=[ctypes.c_void_p,ctypes.c_size_t];k.SecureZeroMemory.restype=ctypes.c_void_p
 return a,k
def encode(value):
...
def write(name,value,persist):
 a,k=_api();raw=encode(value);buf=(ctypes.c_ubyte*len(raw)).from_buffer_copy(raw);c=CREDENTIALW(Type=TYPE_GENERIC,TargetName=name,CredentialBlobSize=len(raw),CredentialBlob=buf,Persist=PERSIST_LOCAL_MACHINE if persist=="unlimited" else PERSIST_SESSION,UserName="GrowthMap")
 try:
  if not a.CredWriteW(ctypes.byref(c),0):raise ctypes.WinError(ctypes.get_last_error())
 finally:k.SecureZeroMemory(buf,len(raw))
def read(name):
 a,_=_api();p=ctypes.POINTER(CREDENTIALW)()

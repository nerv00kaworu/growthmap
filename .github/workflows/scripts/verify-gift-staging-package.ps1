...
$resources='desktop/dist/win-unpacked/resources';$asar=Join-Path $resources 'app.asar';$config=Join-Path $resources 'commercial-config.json';$key=Join-Path $resources 'commercial/license_public_key.pem'
foreach($path in @($asar,$config,$key,'desktop/dist/win-unpacked/GrowthMap.exe')){if(-not(Test-Path $path -PathType Leaf)){throw "Missing staged package file: $path"}}
